...
namespace Runbow.TWS.Dao
{
    public class ProductAccessor : BaseAccessor
    {
        ...
        private string SKUSearchConditionFG(ProductSearchCondition SearchCondition)
        {
            ...
            if (!string.IsNullOrWhiteSpace(SearchCondition.StorerID))
            {
                ...
                if (numbers != null && numbers.Any())
                {
                    sb.Append(" and WMS_ProductFG.StorerID in ( ");
                ...
                {
                    sb.Append(" and WMS_ProductFG.StorerID ='" + SearchCondition.StorerID.Trim() + "' ");
                }
            }
            if (string.IsNullOrWhiteSpace(SearchCondition.StorerID))
            {
                sb.Append(" and WMS_ProductFG.StorerID=null");
            }
            if (!string.IsNullOrEmpty(SearchCondition.SKUClassification))
            {
                sb.Append(" and WMS_ProductFG.SKUClassification='" + SearchCondition.SKUClassification + "'");
            }
            if (!string.IsNullOrEmpty(SearchCondition.SKU))
            {
                ...
                if (SKUList != null && SKUList.Any())
                {
                    sb.Append(" AND  WMS_ProductFG.SKU in ( ");
                ...
                {
                    sb.Append(" AND  WMS_ProductFG.SKU  like '%" + SearchCondition.SKU.Trim() + "%' ");
                }
            }
            ...
            if (!string.IsNullOrEmpty(SearchCondition.Article))
            {
                ...
                if (ArticleList != null && ArticleList.Any())
                {
                    sb.Append(" AND  WMS_ProductFG.Str10 in ( ");
                ...
                {
                    sb.Append(" AND  WMS_ProductFG.Str10  like '%" + SearchCondition.Article.Trim() + "%' ");
                }
            }
            ...
            {
                sb.Append(" and WMS_ProductFG.Str9='" + SearchCondition.Size.Trim() + "'");
            }
            if (!string.IsNullOrWhiteSpace(SearchCondition.Remark))
            {
                sb.Append(" and (WMS_ProductFG.Remark like '%" + SearchCondition.Remark + "%'");
                sb.Append(" or WMS_ProductFG.GoodsName like '%" + SearchCondition.Remark + "%')");
            }
            return sb.ToString();
        }
        ...
        private string SKUSearchCondition(ProductSearchCondition SearchCondition)
        {
            ...
            if (!string.IsNullOrWhiteSpace(SearchCondition.Model) && SearchCondition.Model == "产品")
            {
                sb.Append(" and WMS_Product.Str19='" + SearchCondition.Model.Trim() + "'");
            }
            else
            {
                sb.Append(" and WMS_Product.Str19='" + SearchCondition.Model.Trim() + "'");
        ...
        /// </summary>
        /// <param name="productStorer"></param>
        public ProductStorerInfo GetSKUProductInfoFG(IEnumerable<ProductStorerInfo> productStorer)
        {
            using (SqlConnection conn = new SqlConnection(BaseAccessor._dataBase.ConnectionString))
            {
                string message = "";
                SqlCommand cmd = new SqlCommand("[Proc_WMS_UpdateProductFG]", conn);
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.AddWithValue("@Productdata", productStorer.Select(p => new ProductStorerInfoToDB(p)));
                cmd.Parameters[0].SqlDbType = SqlDbType.Structured;
                cmd.Parameters.AddWithValue("@message", message);
