...
{
    public class NikeReturnAllocate:BaseAllocate
    {
        public NikeReturnAllocate(string AllocateMode, string AllocateType, long ID, string CustomerId,
            string UserName, IEnumerable<PreOrderIds> List, IEnumerable<PreOrderDetail> Pod) : base(AllocateMode, AllocateType, ID, CustomerId, UserName, List, Pod)
        { }
        ...
        public override void CustomerDefinedSettledAllocate()
        {
            ...
            {
                this.SqlProc = "Proc_WMS_AutomatedOutbound_" + CustomerId;
            ...
            {//手动分配
                this.SqlProc = "Proc_WMS_ManualAllocation_" + CustomerId;
                PreOrderService service = new PreOrderService();
                var response = service.ManualAllocationJson(new ManualAllocationRequest() { PodRequest = Pod, ID = ID, CustomerId = CustomerId, Creator = UserName, Criterion = AllocateType, SqlProc = SqlProc });
                if (!response.IsSuccess)
                {
