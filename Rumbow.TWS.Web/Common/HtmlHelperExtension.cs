...
namespace System.Web.Mvc
{
    public static class HtmlHelperExtension
    {
        ...
        public static MvcHtmlString PagerNew(this HtmlHelper helper, int pageIndex, int pageCount, string identify)
        {
            PagerViewModel model = new PagerViewModel() { PageIndex = pageIndex, PageCount = pageCount, Identify = identify };
            return helper.Partial("PagerNew", model);
        }
        ...
        //**********************************chaomin.Song  AngularJS 做准备**********************************//
        public static string anjularJs(this HtmlHelper html,Dictionary<string, string> dit)
        {
            //var anjularStr = new Dictionary<string, object>();
            //anjularStr.Add(attribute, value);
            foreach (var item in dit)
            {
                string anjularStr = string.Format("{0}='{1}'", item.Key, item.Value);
            }
         
            return "";
        }
        public delegate Dictionary<string, object> ProcessanjularJs<T, S>(T s1, S s2);
    }
}