...
namespace System.Web.Mvc
{
    public static class UrlHelperExtension
    {
        ...
        public static MvcHtmlString LoadPageCssJs(this UrlHelper url, bool css = true, bool js = true)
        {
            ...
            if (css)
            {
                sb.AppendLine(Css(url, string.Format("{0}/{1}/{2}.css?param=" + DateTime.Now.ToString(), area, controller, action)).ToString());
            }

            if (js)
            {
                sb.AppendLine(Js(url, string.Format("{0}/{1}/{2}.js?param=" + DateTime.Now.ToString(), area, controller, action)).ToString());
            }
