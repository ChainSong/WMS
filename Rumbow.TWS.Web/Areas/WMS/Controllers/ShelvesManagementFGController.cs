...
{
    public class ShelvesManagementFGController : BaseController
    {
        //
        ...
        public string StagingReceipt(string CustomerID, string Jaonstr)
        {
            ...
            try
            {
                ...
                responses.Each((i, data) =>
                {
                    ...
                    {
                        validation = false;
                        result.Append("").Append("第" + (i + 1).ToString() + "行，【" + data.SKU + "】 产品编码在该客户下不存在！");
                    }
        ...
        public string AddInventory(string id)
        {
            ...
            try
            {
                ...
                logs.Add(operation);
                response = new ShelvesManagementService().AddInventory(id, base.UserInfo.Name, wms.FirstOrDefault().Name,"产品");
                if (response == "成功")
                {
                    new LogOperationService().AddLogOperation(logs);
                }
                        ...
                        for (int j = 0; j < ds.Tables.Count; j++)
                        {
                            if (ds != null && ds.Tables[j] != null)
                            {
                                ...
                                for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                                {
                                    ...
                                    {
                                        return new { result = "第【" + i + "】行,【" + ds.Tables[0].Rows[i]["产品编码"].ToString() + "】 产品编码或者条码在该客户下不存在！", IsSuccess = false }.ToJsonString();
                                    }
