...
{
    public class SettledForJQWLInRunbowProject : BaseSettled
    {
        public SettledForJQWLInRunbowProject(int SettledType, long customerOrShipperID, bool IsGroupedPods, IEnumerable<long> PodIDs, long ProjectID, string Creator, long RelatedCustomerID)
        ...
        public override void CustomerDefinedSettledPod()
        {
            ...
            {
                #region 上海 供应商为嘉庆，出货运单 同天同城同客户
                if (gPod.StartCityName == "上海" && gPod.PODTypeName == "出货运单")
                {
                    var innerPods = gPod.PodIDs.Split('|').Select(i => i.ObjectToInt64());

                    var groupPodResponse = new PodService().SettledPodSearch(new SettledPodRequest() { IDs = innerPods, SettledType = this.settledType, IsID = 1 });
                ...
                else
                {
                    ...
                    this.settledPodCollection.Each((i, settledPod) =>
                    {
                        var originalPodIDs = from q in ids where q == settledPod.PodID.ToString() select q;
                        if (originalPodIDs.Count() > 0)
                        {
                            if (settledPod.Weight == null)
                            {
                            ...
                            {
                                if (originalPodIDs.First().ToString() == settledPod.PodID.ToString() && settledPod.PODTypeName == "移库调拨")
                                {
