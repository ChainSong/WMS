...
{
    public class SettledForSXWLInRunbowProject_New : BaseSettled
    {
        public SettledForSXWLInRunbowProject_New(int SettledType, long customerOrShipperID, bool IsGroupedPods, IEnumerable<long> PodIDs, long ProjectID, string Creator, long RelatedCustomerID)
        ...
        public override void CustomerDefinedSettledPod()
        {
            ...
            if (base.podCollection.First().CustomerID == 7)
            {
                ...
                foreach (var gPod in SettledPodResponse.GroupedPods)
                {
                    ...
                    //        continue;
                    //    }
                    //    shipAmt = gPod.Weight < 300 ? (decimal)(originalPod.Weight / gPod.Weight) * price.Price * 300 : (decimal)originalPod.Weight * price.Price;
                    //}
