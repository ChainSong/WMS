...
{
    public class SettledForDFWLInRunbowProject : ISettledForPod
    {
        ...
        public void SettledPodForPay(IEnumerable<long> PodIDs, string creator, long ProjectID, int Target, long CustomerOrShipperID, long RelatedCustomerID, System.Text.StringBuilder Message)
        {
            ...
            var groupedPodsResponse = service.SettledPodSearch(new SettledPodRequest() { IDs = PodIDs, SettledType = 1, IsID = 1 });
            if (!groupedPodsResponse.IsSuccess)
            {
                throw groupedPodsResponse.Exception;
            }

            var groupedPodsResponses = service.SettledPodSearch(new SettledPodRequest() { IDs = PodIDs, SettledType = 1 });
            if (!groupedPodsResponses.IsSuccess)
            {
                throw groupedPodsResponses.Exception;
            }
            ...
            if (quotedPrice.FirstOrDefault().StartCityName == "上海")
            {
                #region 上海akzo
                ...
                if (podsResponse.Result.First().CustomerID == 7)
                {
                    ...
                    foreach (var gPod in groupedPodsResponse.Result.GroupedPods)
                    {
                        ...
                        //gPods.TtlOrTplName = gPod.TtlOrTplID.ToString();
                        if (tempPrice == null)
                        {
                        ...
                        decimal shipAmt = price.Price * (decimal)gPod.Weight;
                        ...
                        foreach (var id in innerPodIDs)
                        {
                            ...
                            SettledPod settledPod = new SettledPod()
                            {
                                ...
                                BAFAmt = 0,
                                PointAmt = 0,
                                OtherAmt = (decimal)(originalPod.Weight / 1000 * 8),
                                Amt1 = 0,
                                Amt2 = 0,
                                Amt3 = 0,
                                Amt4 = 0,
            ...
            else
            {
                ...
                if (podsResponse.Result.First().CustomerID == 7)
                {
                    relatedCustomerID = 7;
                    //var podsResponses = from q in podsResponse.Result
                    //                    group q by new { q.Str3, q.StartCityName, q.ActualDeliveryDate, q.EndCityName }
                    ...
                    //                            PodIDs = string.Join("|", ids),
                    //                        };
                    foreach (var gPod in groupedPodsResponses.Result.GroupedPods)
                    {
                        double weight = (Double)gPod.Weight;
                        if (weight < 50)
                        {
                        ...
                        foreach (var id in innerPodIDs)
                        {
                            ...
                            SettledPod settledPod = new SettledPod()
                            {
                                ...
                                Volume = originalPod.Volume,
                                GoodsNumber = originalPod.GoodsNumber,
                                ShipAmt = gPod.Weight < 50 ? shipAmt * (decimal)(originalPod.Weight / gPod.Weight) * 50 : shipAmt * (decimal)originalPod.Weight,
                                BAFAmt = 0,
                                PointAmt = 0,
                                OtherAmt = 0,//(decimal)(originalPod.Weight / 1000 * 8),
                                Amt1 = 0,
                                Amt2 = 0,
                                Amt3 = 0,
                                Amt4 = 0,
        ...
        private void Settlement(string settledNumber, long ProjectID, StringBuilder Message, double cube, double boxnumner, IEnumerable<Pod> pods, IList<SettledPod> settledPods, IEnumerable<QuotedPrice> quotedPrices)
        {
            ...
            {
                if (pods.FirstOrDefault().EndCityName == "嘉善" || pods.FirstOrDefault().StartCityName == "嘉善")
                {
                    if (shipAmt < 900)
                    {
                        shipAmt = 900;
