...
{
    public class SettledForCXWLInRunbowProject : ISettledForPod
    {
        private double _settledByItem = 50;
        private decimal _sendAmt = 100;
        ...
        public void SettledPodForPay(IEnumerable<long> PodIDs, string Creator, long ProjectID,int Target,long CustomerOrShipperID,long RelatedCustomerID, StringBuilder Message)
        {
            ...
            foreach (var pod in podsResponse.Result)
            {
                ...
                if (pod.CustomerID == 2)
                {
                    ...
                    q.ProjectID == ProjectID && q.Target == 1 && q.TargetID == pod.ShipperID && q.StartCityID == pod.StartCityID && q.EndCityID == pod.EndCityID
                    && q.PodTypeID == pod.PODTypeID && q.ShipperTypeID == pod.ShipperTypeID && q.TplOrTtlID == pod.TtlOrTplID && q.RelatedCustomerID == pod.CustomerID
                    ...
                    else
                    {
                        ...
                        settledPod.ShipAmt = settledPod.ShipAmt + this._sendAmt;
                    }
