...
{
    public class SettledForXCSWLInRunbowProject : ISettledForPod
    {
        ...
        public void SettledPodForPay(IEnumerable<long> PodIDs, string creator, long ProjectID, int Target, long CustomerOrShipperID, long RelatedCustomerID, System.Text.StringBuilder Message)
        {
            ...
            if (podsResponse.Result.First().CustomerID == 1)
            {
                ...
                foreach (var gPod in groupedPodsResponse.Result.GroupedPods)
                {
                    ...
                    {
                        var temp = (from p in podsResponse.Result.Where(k => innerPodIDs.Contains(k.ID) && k.PODTypeName.Trim() == "调拨运单") group p by p.Str7 into g select new { g.Key, Pods = g });
                        if (temp != null)
                        {
                            temp.Each((i, k) =>
                            {
                                var innPodIDs = k.Pods.Select(p => p.ID);
                                settledPods.First(s => s.PodID == innPodIDs.First()).PointAmt = 0;
                                settledPods.First(s => s.PodID == innPodIDs.First()).Remark += "调拨没有点费";
                            });
                        }
                    }
                }
