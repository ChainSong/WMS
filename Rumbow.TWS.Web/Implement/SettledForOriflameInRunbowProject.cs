...
{
    public class SettledForOriflameInRunbowProject : ISettledForPod
    {
        public void SettledPodForReceive(IEnumerable<long> PodIDs, string Creator, long ProjectID, int Target, long CustomerOrShipperID, long RelatedCustomerID, StringBuilder Message)
        {
            ...
            foreach (var pod in podsResponse.Result)
            {
                ...
                {
                    Message.Append("运单:").Append(pod.CustomerOrderNumber).Append(",对应客户为:").Append(pod.CustomerName).Append(",起运城市为:")
                ...
                if (pod.ShipperID==3)
                {
                    if (pod.StartCityName == "上海" && pod.EndCityName == "南京")
                    {
                        if (pod.Weight.Value <= 1)
                        {
                            settledPod.ShipAmt = 12.00m;
                        }
                        else
                        {
                            settledPod.ShipAmt = (decimal)(12 + (pod.Weight.Value - 1) * 2);
                        }
