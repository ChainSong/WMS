...
{
    public class SettledForTYWLInRunbowProject : ISettledForPod
    {
        ...
        public void SettledPodForPay(IEnumerable<long> PodIDs, string creator, long ProjectID, int Target, long CustomerOrShipperID, long RelatedCustomerID, System.Text.StringBuilder Message)
        {
            ...
            if (podsResponse.Result.First().CustomerID == 7)
            {
                ...
                foreach (var gPod in groupedPodsResponse.Result.GroupedPods)
                {
                    ...
                    q.ProjectID == ProjectID && q.Target == 1 && (q.TargetID == gPod.TargetID || q.TargetID == 2) && q.RelatedCustomerID == relatedCustomerID && q.StartCityName.Trim() == gPod.StartCityName.Trim() && q.EndCityName.Trim() == gPod.EndCityName.Trim()
                    ...
                    foreach (var id in innerPodIDs)
                    {
                        ...
                        SettledPod settledPod = new SettledPod()
                        {
                            ...
                            ShipAmt = gPod.Weight < 300 ? shipAmt * (decimal)(originalPod.Weight / gPod.Weight) * 300 : shipAmt * (decimal)originalPod.Weight,
                            BAFAmt = 0,
                            PointAmt = 0,
                            OtherAmt = 0,
                            Amt1 = 0,
                    ...
                    foreach (var gcPod in groupedCustomerPodsResponse.Result.GroupedPods)
                    {
                        ...
                        if (sumshipAmt < 120)
                        {
                            ...
                            {
                                if (tempPrice.TargetID != 2&&(tempPrice.EndCityName=="北京"||tempPrice.EndCityName=="天津"))
                                {
                                    settledCustomerPodPod.OtherAmt += (120 - sumshipAmt) * (decimal)(settledCustomerPodPod.Weight / gcPod.Weight);
                                }
                            }
