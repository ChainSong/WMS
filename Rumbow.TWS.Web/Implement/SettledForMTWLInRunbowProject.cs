...
{
    public class SettledForMTWLInRunbowProject : ISettledForPod  
    {
                ...
                foreach (var gPod in groupedPodsResponse.Result.GroupedPods)
                {
                    ...
                    foreach (var id in innerPodIDs)
                    {
                        ...
                        SettledPod settledPod = new SettledPod()
                        {
                            ...
                            ShipAmt = weight < 300 ? shipAmt * ((decimal)originalPod.Weight / (decimal)weight) * 300 : (decimal)originalPod.Weight * shipAmt,
                            BAFAmt = 0,
                            PointAmt = 0,
                            OtherAmt = 0,
                            Amt1 = 0,
