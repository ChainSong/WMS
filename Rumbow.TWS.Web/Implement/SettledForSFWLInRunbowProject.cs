...
{
    public class SettledForSFWLInRunbowProject : ISettledForPod
    {
                        ...
                        if (temp != null)
                        {
                            temp.Each((i, k) =>
                            {
                                if (i > 0)
                                {
                                    ...
                                    settledPods.First(s => s.PodID == innPodIDs.First()).Remark = "盛丰出货，同天同城同客户30元点费";
                        ...
                        if (temp != null)
                        {
                            foreach (var k in temp)
                            {
                                if (k.Key == "经销商")
                                {
                                    ...
                                    settledPods.First(s => innPodIDs.Contains(s.PodID)).Remark = "盛丰退货，同天同城经销商30元点费";
                                    break;
                                }
                            }
                        }
