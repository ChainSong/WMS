$(document).ready(function () {
    ...
    $('#insertTransportationLine').click(function () {
        if ($('#StartPlaceID').val() === '') {
            Runbow.TWS.Alert('请选择起运地');
            return;
        }

        if ($('#EndPlaceID').val() === '') {
            Runbow.TWS.Alert('请选择目的地');
            return;
        }

        $.send(
            '/ShipperManagement/ShipperManagement/InsertCRMShipperTransportationLine',
    ...
    $('#resultTable').find('.deleteCRMShipperTransportationLine').live('click', function () {
        ...
        $.send(
            '/ShipperManagement/ShipperManagement/DeleteCRMShipperTransportationLine',
            { id: id },
            function (response) {
                if (response.IsSuccess) {
                    tr.remove();
