$(document).ready(function () {
    ...
    function ReloadAjax() {
        if ($("#WorkStationId").val() != "") {
            $.ajax({
                type: "POST",
                url: "/WMS/IntelligentOperation/GetMessageMQ_Receipt",
                data: {
                    WorkStationId: $("#WorkStationId").val()
                },
                async: "false",
                success: function (datas) {
                    if (datas.Code == 1) {
                        ...
                        for (var i = 0; i < datas.instructions.length; i++) {
                            ...
                            <div>SKU：" + datas.instructions[i].SKU + "</div> <div class='ExpectQty'>数量：" + datas.instructions[i].QtyExcepted + "</div>\
    ...
    $("#Ack").click(function () {
        ...
        if ($(".ActualQty:first-child").val() > 0 || $(".ActualQty:first-child").val() == undefined) {
            if ($(".ActualQty:first-child").val() > 0) {
                var ExpectQty = $(".ExpectQty")[0].innerText.substring(3, $(".ExpectQty")[0].innerText.length);
                var ActualQty = $(".ActualQty:first-child").val();
                if (ActualQty < ExpectQty) {
                    layer.confirm('<font size="4">实收数量小于期望数量，是否立即补发指令？</font>', {
            ...
            if ($("li:first-child").length > 0) {
                $.ajax({
                    type: "POST",
                    url: "/WMS/IntelligentOperation/SubmitData_Receipt",
                    data: {
    ...
    var IntelligentPickGoods = {
        ...
        ResetFun: function () {
            ...
            if ($("#WorkStationId").val() != "") {
                $.ajax({
                    type: "POST",
                    url: "/WMS/IntelligentOperation/CloseChannel_Receipt",
                    data: {
                        WorkStationId: $("#WorkStationId").val()
                    },
                    async: "false",
