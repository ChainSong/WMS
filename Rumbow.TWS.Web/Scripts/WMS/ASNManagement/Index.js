$(document).ready(function () {
    ...
    $('select[id=ASNCondition_CustomerID]').live('change', function () {
        window.location.href = "/WMS/ASNManagement/Index/?customerID=" + $(this).val();
    });
...
//编辑操作
function edit(ID) {
    location.href = "/WMS/ASNManagement/ASNCreateOrEdit/?ID=" + ID + "&ViewType=2"
}
//单条转入库单操作
...
function createasn() {
    ...
    location.href = "/WMS/ASNManagement/ASNCreateOrEdit/?ID=0" + "&customerID=" + customerid + "" + "&ViewType=1"
}

function ViewReceipt(ID, CustomerID) {
    $.ajax({
        type: "POST",
        url: "/WMS/ASNManagement/CountReceipt",
        data: {
            "ID": ID
        },
        async: "false",
...
function addNew(td11, td22, td33) {
    ...
    td2.append($("<a href=" + '/WMS/ReceiptManagement/ReceiptCreate/?ID=' + td11 + '&ViewType=3>' + td22 + "</a>"));
    td3.append($("<b>" + td33 + "</b>"));
    row.append(td2);
    row.append(td3);
    table1.append(row);
