...
$('select[id=CustomerID]').live('change', function () {
    window.location.href = "/WMS/ASNManagement/ASNCreateOrEdit/?ID=0" + "&customerID=" + $(this).val() + "&ViewType=1";
});
...
//从view页面到edit页面
function edits(ID) {
    location.href = "/WMS/ASNManagement/ASNCreateOrEdit/?ID=0" + ID + "&ViewType=2"
}
...
function AddASNAndASNDetails() {
    ...
    $.ajax({
        type: "POST",
        url: "/WMS/ASNManagement/AddASNAndASNDetails",
        data: {
            "JsonTable": JsonTable,
        ...
        success: function (data) {
            if (data == "外部单号已存在") {
                ...
                location.href = "/WMS/ASNManagement/ASNCreateOrEdit/?ID=" + d + "&ViewType=3";
            } else {
...
function UpdateASNAndASNDetail(asndetailcount) {
    ...
    $.ajax({
        type: "Post",
        url: "/WMS/ASNManagement/UpdateASNAndASNDetails",
        data: {
            "JsonTable": JsonTable,
            "ID": ID,
            "ExternReceiptNumber": ExternReceiptNumber,
        ...
        async: "false",
        success: function (data) {
            location.href = "/WMS/ASNManagement/ASNCreateOrEdit/?ID=" + ID + "&ViewType=3";
            showMsg("修改成功!", "4000");
        },
        error: function (msg) {
            showMsg("修改失败！", "4000");
