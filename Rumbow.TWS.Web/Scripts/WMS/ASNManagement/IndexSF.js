$(document).ready(function () {
    ...
    $('select[id=ASNCondition_CustomerID]').live('change', function () {
        window.location.href = "/WMS/ASNManagement/IndexSF/?customerID=" + $(this).val();
    });
    ...
    $('#portButton').live('click', function () {
        ...
        if (a == "") {
            ...
            window.location.href = "/WMS/ASNManagement/ExportOrder?IDs=" + a + "&CustomerID=" + $("#ASNCondition_CustomerID").val();
            return false;
        }
    });
    //生成上架库位
    ...
    $('#ReceiptStatusStatis').live('click', function () {
        var href = "/WMS/ASNManagement/ReceiptStatusStatis?CustomerID=" + $("#ASNCondition_CustomerID").val() + "";
        layer.open({
            type: 2,
            title: '入库单状态统计',
            shadeClose: true,
...
var fileImportClick = function () {
    ...
    $.ajaxFileUpload({
        url: '/WMS/ASNManagement/ImportASN',
        secureuri: false,
...
//编辑操作
function edit(ID) {
    location.href = "/WMS/ASNManagement/ASNCreateOrEditSF/?ID=" + ID + "&ViewType=2"
}
//单条转入库单操作
...
function ASNDelete(ID) {
    layer.confirm('<font size="4">是否取消？</font>', {
        ...
        if (IDs != "" && IDs != 0) {
            $.ajax({
                type: "POST",
                url: "/WMS/ASNManagement/ASNDelete",
                data: {
                    "ID": IDs
                },
                async: "false",
    ...
    layer.confirm('<font size="4">是否批量转入库单？</font>', {
        ...
        if ($("#resultTable tbody input[type='checkbox']:checked").size() == 0) {
            ...
            if (count == 0) {
                ...
                $.ajax({
                    type: "POST",
                    url: "/WMS/ASNManagement/InsertIntoReceiptAndReceiptDetails",
                    data: {
                        "ASNIDs": asnid
                    },
                    async: "false",
    ...
    layer.confirm('<font size="4">是否批量取消？</font>', {
        ...
        if ($("#resultTable tbody input[type='checkbox']:checked").size() == 0) {
            ...
            if (count == 0) {
                ...
                $.ajax({
                    type: "POST",
                    url: "/WMS/ASNManagement/ASNStatusReturn",
                    data: {
                        "asnnumberlist": asnid
                    },
                    async: "false",
    ...
    layer.confirm('<font size="4">是否批量完成？</font>', {
        ...
        if ($("#resultTable tbody input[type='checkbox']:checked").size() == 0) {
            ...
            if (count == 0) {
                ...
                $.ajax({
                    type: "POST",
                    url: "/WMS/ASNManagement/CompletALLSelect",
                    data: {
                        "asnid": asnid
                    },
                    async: "false",
...
function createasn() {
    ...
    location.href = "/WMS/ASNManagement/ASNCreateOrEditSF/?ID=0" + "&customerID=" + customerid + "" + "&ViewType=1"
}

function ViewReceipt(ID, CustomerID) {
    $.ajax({
        type: "POST",
        url: "/WMS/ASNManagement/CountReceiptSF",
        data: {
            "ID": ID
        },
        async: "false",
        success: function (data) {
            if (data.length.toString() == "1") {
                location.href = "/WMS/ReceiptManagement/ReceiptCreate/?ID=" + data[0].ID + "&ViewType=3&CustomerID=" + CustomerID
            }
...
function addNew(td11, td22, td33) {
    ...
    td2.append($("<a href=" + '/WMS/ReceiptManagement/ReceiptCreateSF/?ID=' + td11 + '&ViewType=3>' + td22 + "</a>"));
    td3.append($("<b>" + td33 + "</b>"));
    row.append(td2);
    row.append(td3);
    table1.append(row);
