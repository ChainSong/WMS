$(document).ready(function () {
    if ($('#LogOperationCondition_CustomerID').val() == "") {
        $('#LogOperationCondition_CustomerID option:first').next().attr("selected", "selected");
    ...
    $('.DynamicCalendarRange').each(function (index) {
        ...
        var descID = 'LogOperationCondition_';
        if (pref === 'start') {
    ...
    var setPageControlVal = function () {
        $('.notKeyVal').each(function (index) {
            var id = $(this).attr("id");
            var descId = "LogOperationCondition_" + id;
        ...
        $('.DropDownList').each(function (index) {
            var id = $(this).attr("id");
            var descId = "LogOperationCondition_" + id.substr(0, id.length - 4) + "ID";
            $('#' + descId).val($(this).val());
        });
