$(document).ready(function () {
    ...
    $('#searchButton').click(function () {
        setPageControlVal();
        var customerName = $('#SearchCondition_CustomerID').val();
        var warehouseName = $('#SearchCondition_Warehouse').val();
...
function SearchDetail(status, obj, type) {
    ...
    $.ajax({
        type: "Post",
        url: "/WMS/OrderECManagement/SearchOrderTotal",
        data: {
            "CustomerID": $('#SearchCondition_CustomerID').val(),
            "Warehouose": $('#SearchCondition_Warehouse').val(),
            "Status": status,
            "StartTime": $('#SearchCondition_StartCreateTime').val(),
            "EndTime": $('#SearchCondition_EndCreateTime').val(),
            "Type": type
        },
        async: "false",
        success: function (data) {
            if (data.Errorcode == 1) {
                ...
                for (var i = 0; i < OrderInfo.data.length; i++) {
                    ...
                    html += "<td>" + obj.ExternOrderNumber + "</td>";
                    html += "<td>" + obj.str1 + "</td>";
                    html += "<td>" + obj.Int1 + "</td>";
                    html += "<td>" + obj.OrderType + "</td>";
                    html += "<td>" + obj.Warehouse + "</td>";
                    html += "</tr>"
                }
                $('#tbList').append(html);
