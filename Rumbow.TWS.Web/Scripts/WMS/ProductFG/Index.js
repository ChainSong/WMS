$(document).ready(function () {
    ...
    $(".AddButton").live("click", function () {
        ...
        window.location.href = "/WMS/ProductFG/productdetail?ID=" + skuid;
    })
    //编辑
    $('.editSettledPod').live('click', function () {
        ...
        if (CustomerId != "") {
            location.href = "/WMS/ProductFG/AddProduct/?ID=" + settledpod + "&typeid=3&CustomerId=" + CustomerId;
        } else {
            location.href = "/WMS/ProductFG/AddProduct/?ID=" + settledpod + "&typeid=3";
        }
    })
})
...
$(function () {
    ...
    $('#AddButton').click(function () {
        var CustomerId = $("#ProductSKU_StorerID")[0].value;
        if (CustomerId != "") {
            window.location.href = "/WMS/ProductFG/AddProduct?typeid=1&CustomerId=" + CustomerId;
        } else {
            window.location.href = "/WMS/ProductFG/AddProduct?typeid=1";
        }
    });
    $('#ExeclButton').click(function () {
        window.location.href = "/WMS/ProductFG/ExeclProduct";
    });
    //下载模板
    $("#portButtonTemplet").click(function () {
        var CustomerId = $("#ProductSKU_StorerID")[0].value;
        demo('/WMS/ProductFG/ProductdemoExecl/?CustomerId=' + CustomerId);
    ...
    $("#importDownCoatButton").click(function () {
        ...
        layer.confirm('是否更新羽绒服标记？', {
            ...
            //WebPortal.MessageMask.Show("导入中...");
            $.ajaxFileUpload({
                url: '/WMS/ProductFG/importDownCoat',
                secureuri: false,
                fileElementId: 'importExcel',
                dataType: 'json',
                data: { StorerID: $("#StorerID").val(), pathFile: $("#importExcel").val() },
...
var fileImportClick = function () {
    ...
    layer.confirm('是否导入SKU？', {
        ...
        //WebPortal.MessageMask.Show("导入中...");
        $.ajaxFileUpload({
            url: '/WMS/ProductFG/ExeclProduct',
            secureuri: false,
            fileElementId: 'importExcel',
            dataType: 'json',
