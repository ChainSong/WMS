    ...
    $("#ScanSKU").live('keydown', function () {
        if (event.keyCode == 13) {
            ...
            if ($("#chkZero").get(0).checked == true) {
                ...
                //if ($("#CustomerID").val().toString().indexOf("108") > -1)
                if ($("#CustomerName").val().toString().indexOf("吉特WMS") > -1)
                {
                    $(this).val($(this).val());
                }
                else {
...
function QtyChanged(obj) {
    ...
    $.each(orderdetaillist, function (index, item) {
        if (singleSKU == item.SKU && singleQty == item.Qty) {
            ...
            showMsg("产品：" + singleSKU + "数量超出，请检查！", "4000");
            PlaySound();
            //return;
        }
    });
...
function printboxmDiv(id, type, OrderID) {
    ...
    if (packageNumBegin <= packageNumEnd && packageNumBegin <= maxPackageNum && packageNumEnd <= maxPackageNum && !(packageNumBegin == '') && !(packageNumEnd == '') && packageNumBegin > 0 && packageNumEnd > 0) {
        ...
        //}
        //else if (($("#CustomerID").val().toString().indexOf("103") > -1)) {
        //    var href = "/WMS/NikeOSRBJPrint/PrintBoxm_TH?id=" + id + "&type=" + type + "&OrderID=" + OrderID;
        //}
        //else {
