...
$(document).ready(function () {
    ...
    $("#submitButton").live('click', function () {
        ...
        $(".counts").each(function (a, b) {
            ...
            for (var j = 1; j < row2.length; j++) {
                ...
                for (var i = 0; i < col.length; i++) {
                    ...
                    if (i != 5) {
                        if (i == 4) {
                            ...
                            //r2 += "\"" + tds[i].childNodes[0].id.trim() + "\"\:\"" + tds[i].childNodes[1].value + "\",";
                        }
                        else {
                            r2 += "\"" + tds[i].childNodes[0].id.trim() + "\"\:\"" + tds[i].childNodes[0].value + "\",";
                        }
                ...
                if (tds[4].childNodes[1].value == undefined) {
                    r2 += "\"" + "BarCodes" + "\"\:\"" + tds[4].childNodes[2].value + "\",";
                } else {
                    r2 += "\"" + "BarCodes" + "\"\:\"" + tds[4].childNodes[1].value + "\",";
                }
        ...
        if (flag == 0) {
            $.ajax({
                type: "Post",
                url: "/WMS/OrderManagementFG/BarCodePackage",
                data: {
                    "ID": $("#OrderID").val(),
                ...
                success: function (data) {
                    if (data == "") {
                        ...
                        showMsg("保存成功！", 4000);
                        location.href = "/WMS/OrderManagementFG/BarCodePackage/?ID=" + $("#OrderID").val();
                    }
                    else {
    ...
    $("#addButton").live('click', function () {
        ...
        //$.ajax({
        //    type: "Post",
        //    url: "/WMS/OrderManagementFG/GetMaxBoxnumber",
        //    data: {
        //        "OrderID": $("#OrderID").val()
        //    },
        //    async: false,
    ...
    $("#ScanSKU").live('keydown', function () {
        if (event.keyCode == 13) {
            ...
            if (list_json.length==0) {
                $.post("/WMS/OrderManagementFG/GetSKUAndBarCodeByBarCode", { BarCode: inputsku.val() }, function (result) {
                    if (result.length > 0) {
                        $.each(result, function (i, obj) {
                            list_json.push(obj);
                            if (obj["BarCode"] == inputsku.val()) {                                
                                inputsku_val = obj["SKU"];
                            }
                        });
                    }
                    AppendItem(inputsku_val,inputsku,barcode);
                }, "json");
            } else {
                //先判断barcode是否在当前sku_barcode集合中
                $.each(list_json, function (i, obj) {
                    if (obj["BarCode"] == inputsku.val()) {                                
                        inputsku_val = obj["SKU"];                                
                        return false;
                    }
                ...
                } else { AppendItem(inputsku_val,inputsku,barcode);}
            }
        }
    });
...
function CheckBarcode(barcode) {
    ...
    $("input[name='Barcodes']").each(function () {
        ...
        $.each(barcode_val.split(','), function (i, obj) {
            if (obj == barcode) {
                bl = false;
                return false;
            }
...
function AppendItem(inputsku_val,inputsku,barcode) {
    if (CheckQTYOver(inputsku_val)) {
    ...
    if (rows_count == 1 && $(thistr.rows[1]).children().eq(0).find("input").val().trim() == "") {
        if (CheckSKU(inputsku_val)) {
            ...
            $(thistr.rows[1]).children().eq(4).find("input:text").val(1);
            $(thistr.rows[1]).children().eq(4).find("input:hidden").val(barcode);
        }
        else {
            showMsg("SKU不存在!", "4000");
            PlaySound();
    ...
    for (var i = 1; i <= rows_count; i++) {
        if ($(thistr.rows[i]).children().eq(0).find("input").val().trim() == inputsku_val.trim() || $(thistr.rows[i]).children().eq(1).find("input").val().trim() == inputsku_val.trim()) {
            Isexist = true;
            Isexistline = i;
        }
    }
    if (Isexist) {
        var qty = parseInt($(thistr.rows[Isexistline]).children().eq(4).find("input:text").val().trim()) + 1;
        $(thistr.rows[Isexistline]).children().eq(4).find("input:text").val(qty);

        var barcode_val = $(thistr.rows[Isexistline]).children().eq(4).find("input:hidden").val();
        //判断在这个barcodelist中是否存在
        $(thistr.rows[Isexistline]).children().eq(4).find("input:hidden").val(barcode_val + "," + barcode);
    }
    else {
        if (CheckSKU(inputsku_val)) {
            ...
            $(thistr.rows[rows_count]).children().eq(4).find("input:text").val(1)
            $(thistr.rows[rows_count]).children().eq(4).find("input:hidden").val(barcode);
...
function addNew(o) {
    ...
    td4.append($("<input type='hidden' name='Barcodes' />"));
...
function DeletePackInfo(packagenumber) {
    ...
    $.ajax({
        type: "Post",
        url: "/WMS/OrderManagementFG/DeletePackInfo",
...
function printboxDiv(id, type, OrderID) {
    if ($("#CustomerName").val().toString().indexOf("NIKE-OSR") > -1) {
        window.location.href = "/WMS/NikeOSRBJPrint/PrintBox?id=" + id + "&type=" + type + "&OrderID=" + OrderID;
    }
    else if (($("#CustomerName").val().toString().indexOf("NIKE-NFS") > -1) || ($("#CustomerName").val().toString().indexOf("NIKE-CD") > -1)) {
        window.location.href = "/WMS/NikeNFSPrint/PrintBox?id=" + id + "&type=" + type + "&OrderID=" + OrderID;
    }
    else if ($("#CustomerName").val().toString().indexOf("YXDR") > -1) {
        window.location.href = "/WMS/YXDRPackagePrint/PrintBoxList?id=" + id + "&type=" + type + "&OrderID=" + OrderID;
    }
    else {
        window.location.href = "/WMS/NikeOSRBJPrint/PrintBox?id=" + id + "&type=" + type + "&OrderID=" + OrderID;
    }
...
function printboxmDiv(id, type, OrderID) {
    if ($("#CustomerName").val().toString().indexOf("YXDR") > -1) {
        window.location.href = "/WMS/YXDRPackagePrint/PrintCarton?id=" + id + "&type=" + type + "&OrderID=" + OrderID;
    }
    else if ($("#CustomerName").val().toString().indexOf("NIKE-CD") > -1) {
        window.location.href = "/WMS/NikeOSRBJPrint/PrintBoxCD?id=" + id + "&type=" + type + "&OrderID=" + OrderID;
    }
    else {
        window.location.href = "/WMS/NikeOSRBJPrint/PrintBoxm?id=" + id + "&type=" + type + "&OrderID=" + OrderID;
    }
