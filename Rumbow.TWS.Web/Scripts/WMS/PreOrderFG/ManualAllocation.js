...
$(document).ready(function () {
    ...
    $(".LocationChildren").live("dblclick", function () {
        var self = this;
        $.ajax({
            url: "/WMS/PreOrderFG/GetPrdOrder_distributionInventory",
            type: "POST",
            dataType: "json",
            // 注意这里
            async: false,
    ...
    $("#searchButton").click(function () {
        ...
        $.ajax({
            type: "POST",
            url: "/WMS/PreOrderFG/ManualAllocationJson",
            ...
            success: function (data) {
                if (data.Errorcode == 1 && data.data[0].Message != "库存不足") {
                    window.location.href = "/WMS/PreOrderFG/PreOrderFGCreateOrEdit/?ID=" + $("#PreAndDetail_SearchCondition_ID")[0].value + '&ViewType=0&Flag=1';
                    //         ?ID=@item.ID&ViewType=0"
                    //post('/WMS/PreOrderFG/PreOrderFGCreateOrEdit', { ID: $("#PreAndDetail_SearchCondition_ID")[0].value, ViewType: '0' });
    ...
    $("#saveButton").click(function () {
        ...
        $.ajax({
            type: "POST",
            url: "/WMS/PreOrderFG/ManualAllocationSaveJson",
            ...
            success: function (data) {
                if (data.Errorcode == 1) {
                    window.location.href = "/WMS/PreOrderFG/PreOrderFGCreateOrEdit/?ID=" + $("#PreAndDetail_SearchCondition_ID")[0].value + '&ViewType=0&Flag=2';
                    //         ?ID=@item.ID&ViewType=0"
                    //post('/WMS/PreOrderFG/PreOrderFGCreateOrEdit', { ID: $("#PreAndDetail_SearchCondition_ID")[0].value, ViewType: '0' });
                } else {
                    showMsg("操作失败！", 2000);
