$(document).ready(function () {
    ...
    $('select[id=SearchCondition_CustomerID]').live('change', function () {
        ...
        window.location.href = "/WMS/PreOrderFG/Index/?customerID=" + $(this).val() + "&hideActionButton=" + hiddenActionButton + "&showEditRelated=" + showEditRelated;
    });
    $("#AddPreOrder").click(function () {
        ...
        window.location.href = "/WMS/PreOrderFG/PreOrderCreateOrEdit/?ViewType=1&customerID=" + $("#SearchCondition_CustomerID").val() + "&WarehouseID=" + WarehouseID;
    });
    ...
    $("#CompleteProOrder").live("click", function () {
        layer.confirm('<font size="4">确认是否批量完成？</font>', {
            ...
            $.ajax({
                ...
                success: function (data) {
                    showMsg("操作成功!", "4000");
                    location.href = "/WMS/PreOrderFG/Index"
                    //location.reload();
                },
                error: function (msg) {
                    showMsg("操作失败", "4000");
    ...
    function demo(url) {
        ...
        var url = '/WMS/PreOrderFG/ReportExportExecl';
        var form = $("<form>");
    ...
    $('#Export').live('click', function () {
        ...
        if (a == "") {
            ...
            window.location.href = "/WMS/PreOrderFG/ExportPreOrder?IDS=" + a + "&CustomerID=" + $('#SearchCondition_CustomerID').val();
            return false;
        }
    })
    ////模拟查询
...
var fileImportClick = function () {
    ...
    //    url1="/WMS/PreOrderFG/ImputEcecl_TH";
    //}
    //else {
    //    url1= "/WMS/PreOrderFG/ImputEcecl";
    //}
    WebPortal.MessageMask.Show("导入中...");
    $.ajaxFileUpload({
        /*url: url1,*/
        url: "/WMS/PreOrderFG/ImputEcecl",
        secureuri: false,
...
function WorkersAlloctions(id, self) {
    layer.confirm('<font size="4">确认是否现场分配？</font>', {
        ...
        layer.close(index);
        $.ajax({
            url: "/WMS/PreOrderFG/WorkersAlloctions",
            type: "POST",
            dataType: "text",
            data: {
                ids: id,
...
function Edits(ID) {
    location.href = "/WMS/PreOrderFG/PreOrderCreateOrEdit/?ID=" + ID + "&ViewType=2"
}
function ManuAlloctions(ID) {
    location.href = "/WMS/PreOrderFG/ManualAllocation/?ID=" + ID + "&ShowSubmit=" + $("#ShowSubmit").val()
}
function AssignedAllocation(ID) {
    location.href = "/WMS/PreOrderFG/AssignedAllocation/?ID=" + ID + "&ShowSubmit=" + $("#ShowSubmit").val()
}
