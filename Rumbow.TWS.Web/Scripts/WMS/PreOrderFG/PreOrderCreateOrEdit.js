$(document).ready(function () {
    ...
    if ($('#ViewType').val() == 2) {
        ...
        for (var i = 1; i < lens; i++) {
            ...
            $.ajax({
                url: "/WMS/PreOrderFG/GetBatchlist",
                type: "POST",
                dataType: "json",
                async: false,
    ...
    $("#ReturnIndex").live("click", function () {
        if ($("#ReturnViewType").val() == "get") {
            ...
            window.location.href = "/WMS/PreOrderFG/Index";
        }
    });
    ...
    $('select[id=CustomerID]').live('change', function () {
        ...
        window.location.href = "/WMS/PreOrderFG/PreOrderCreateOrEdit/?customerID=" + $(this).val() + "&hideActionButton=" + hiddenActionButton + "&showEditRelated=" + showEditRelated + "&ViewType=1";
    });
    $("#CheckOutboundOrder").live("click", function () {
        $.ajax({
            url: "/WMS/PreOrderFG/CheckOutboundOrder",
            type: "POST",
            dataType: "json",
            data: {
    ...
    $("#searchButton").click(function () {
        ...
        $.ajax({
            url: "/WMS/PreOrderFG/AddPreOrderCreateOrEdit",
            type: "POST",
            dataType: "json",
            data: {
            ...
            success: function (data) {
                if (data.Errorcode == 1) {
                    window.location = "/WMS/PreOrderFG/PreOrderCreateOrEdit/?ID=" + data.ID + "&ViewType=0";
                } else {
    ...
    $("#AutomaticAllocation").live('click', (function () {
        layer.confirm('<font size="4">确认是否自动分配</font>', {
            ...
            layer.close(index);
            $.ajax({
                url: "/WMS/PreOrderFG/AutomaticAllocation",
                type: "POST",
                dataType: "text",
                data: {
                ...
                success: function (data) {
                    ...
                    if (StrHtml.data == '操作失败') {
                        ...
                        if (StrHtml.data[0].Note == "9") {
                            showMsg('操作成功', 4000);
                            $("#actionButtonDiv").innerHTML = "<a class='btn btn-success' style='text-decoration: none; color: white' hear='/WMS/PreOrderFG/Index'>返回</a>"
    ...
    $("#ManualAllocation").live('click', (function () {
        window.location.href = "/WMS/PreOrderFG/ManualAllocation/?ID=" + $("#PreAndDetail_SearchCondition_ID").val();
    }));
})
var ListNums = 0;;
var SKUListNums = 0;
...
$('.skucheck').live('dblclick', function () {
    var self = this;
    openPopup("Prepop", true, 1000, 600, '/WMS/ProductFG/index/?flag=1&customerID=' + $('select[id=CustomerName]')[0].value, null, function (Sku, GoodsName, UPC) {
        $(self)[0].value = Sku;
...
$('.upccheck').live('dblclick', function () {
    var self = this;
    openPopup("Prepop", true, 1000, 600, '/WMS/ProductFG/index/?flag=1&customerID=' + $('select[id=CustomerID]')[0].value, null, function (Sku, GoodsName, UPC) {
        $(self)[0].value = UPC;
        $($(self).parent().parent()).children(".GoodsName").children("input")[0].value = GoodsName;
...
//$("#AddPreOrderFG").click(function () {
//    window.location.href = "/WMS/PreOrderFG/PreOrderFGCreateOrEdit";
//});
function Cancel(self) {
    ...
    layer.confirm('<font size="4">确认是否取消</font>', {
        ...
        $.ajax({
            ...
            success: function (data) {
                window.location = "/WMS/PreOrderFG/Index"
            },
            error: function (msg) {
                showMsg("操作失败", "4000");
                window.location = "/WMS/PreOrderFG/Index"
...
function Complete(Id) {
    layer.confirm('<font size="4">确认是否完成</font>', {
        ...
        layer.close(index);
        $.ajax({
            url: "/WMS/PreOrderFG/OrderFinish",
            type: "POST",
            dataType: "json",
            data: {
                ids: Id,
            ...
            success: function (data) {
                showMsg("操作成功!", "4000");
                location.href = "/WMS/PreOrderFG/OrderFinish/ids=" + Id;
            },
            error: function (msg) {
                showMsg("操作失败", "4000");
            }
