$(document).ready(function () {
    ...
    $('select[id=ASNCondition_CustomerID]').live('change', function () {
        window.location.href = "/WMS/ASNManagementSF/Index/?customerID=" + $(this).val();
    });
        ...
        if (a == "") {
            ...
            layer.confirm('<font size="4">是否生成上架库位？</font>', {
                ...
                if (IDs != "" && IDs != 0) {
                    $.ajax({
                        type: "POST",
                        url: "/WMS/ASNManagementSF/CreateShelfLocation",
                        data: {
                            "ID": IDs
                        },
                        async: "false",
    ...
    $("#printShelfLocation").live("click", function () {
        layer.confirm('<font size="4">确认是否打印箱标贴？</font>', {
            ...
            if (id == "") {
                ...
                window.location.href = '/WMS/ASNManagementSF/PrintBoxLabel?ids=' + id + "&type=3";
    ...
    //        title: ['提示', 'font-size:18px;']
    //    },
    //        location.href = '/WMS/ASNManagementSF/PrintASNNewBoxLabel?ids=' + str.substring(0, str.length - 1) + "&type=2"
    //    );
    //    return false;
    //}
...
//编辑操作
function edit(ID) {
    location.href = "/WMS/ASNManagementSF/ASNCreateOrEdit/?ID=" + ID + "&ViewType=2"
}
//单条转入库单操作
function Receipt(ID, CustomerID) {
    $.ajax({
        type: "POST",
        url: "/WMS/ASNManagementSF/TurnCG",
        data: {
...
function ASNDelete(ID) {
    layer.confirm('<font size="4">是否取消？</font>', {
        ...
        if (IDs != "" && IDs != 0) {
            $.ajax({
                type: "POST",
                url: "/WMS/ASNManagementSF/ASNDelete",
                data: {
                    "ID": IDs
                },
                async: "false",
...
function Complet(ID) {
    layer.confirm('<font size="4">是否完成？</font>', {
        ...
        if (ID != "" && ID != 0) {
            $.ajax({
                type: "POST",
                url: "/WMS/ASNManagementSF/Complet",
                data: {
                    "ID": ID
                },
                async: "false",
...
function ViewReceipt(ID, CustomerID) {
    $.ajax({
        type: "POST",
        url: "/WMS/ASNManagementSF/CountReceipt",
        data: {
            "ID": ID
        },
        async: "false",
