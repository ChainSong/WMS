...
$(document).ready(function () {
    ...
    $("#ReceiptSend").live('click', function () {
        ...
        layer.confirm('<font size="4">确认是否批量推送入库单？</font>', {
            ...
            $.ajax({
                url: "/WMS/ReceiptManagement/ReceiptSend",
                type: "POST",
                dataType: "json",
                data: {
    ...
    $("#ReceiptTask").live("click", function () {
        layer.confirm('<font size="4">确认是否下发任务？</font>', {
            ...
            if (id == "") {
                ...
                $.ajax({
                    type: "Post",
                    url: "/WMS/ReceiptManagement/ReceiptTask",
                    data: {
                        "ID": id
                    },
                    async: "false",
    ...
    $("#printButton").live("click", function () {
        layer.confirm('<font size="4">确认是否批量打印上架单？</font>', {
            ...
            if ($("#ProjectName").val() == "NIKE") {
                ...
                location.href = '/WMS/ReceiptManagement/PrintShelvesYXDR?rid=' + str.substring(0, str.length - 1) + "&Flag=1";
            }
            else if ($("#ProjectName").val() == "延锋百利得") {
                location.href = '/WMS/ReceiptManagement/PrintShelvesYFBLD?rid=' + str.substring(0, str.length - 1) + "&Flag=1";
            }
            else if ($("#ProjectName").val() == "Mono" || $("#ProjectName").val() == "吉特") {
                location.href = '/WMS/ReceiptManagement/PrintShelves_JT?rid=' + str.substring(0, str.length - 1) + "&Flag=1";
            }
            else {
                location.href = '/WMS/ReceiptManagement/PrintShelves_JT?rid=' + str.substring(0, str.length - 1) + "&Flag=1";
            }
    ...
    $('#exportButtom').live('click', function () {
        ...
        if (a == "") {
            ...
            window.location.href = "/WMS/ReceiptManagement/ExportOrder?IDs=" + a + "&CustomerID=" + $("#SearchCondition_CustomerID").val();
            //$.ajax({
    ...
    $('#exportButton').live('click', function () {
        ...
        if (a == "") {
            ...
            window.location.href = "/WMS/ReceiptManagement/ExportShelves?IDs=" + a + "&CustomerID=" + $("#SearchCondition_CustomerID").val() + "&type=1";
            return false;
        }
    });
    $('#exportShelvesButton').live('click', function () {
        ...
        if (a == "") {
            ...
            window.location.href = "/WMS/ReceiptManagement/ExportShelves?IDs=" + a + "&CustomerID=" + $("#SearchCondition_CustomerID").val() + "&type=2";
            return false;
        }
    });
...
$("#addButton").live('click', function () {
    ...
    location.href = "/WMS/ReceiptManagement/ReceiptCreate/?ViewType=0&CustomerID=" + CustomerID
});
...
function BackCloseBox(ExternNumber) {
    layer.confirm('<font size="4">确认是否回退装箱数据？</font>', {
        ...
        //按钮
            ...
            $.ajax({
                type: "POST",
                url: "/WMS/ReceiptManagement/BackCloseBox",
                data: {
...
function Shelves(ID, type) {
    $.ajax({
        type: "Post",
        url: "/WMS/ReceiptManagement/ValidOrderCancel",
        data: {
            ...
            if (data == "该订单已取消") {
                ...
                if ($("#ProjectName").val() == "爱库存" && type == "快进快出") {
                    layer.confirm('<font size="4">此订单为快进快出订单，点击确定则加入库存？</font>', {
                        ...
                        $.ajax({
                            type: "POST",
                            url: "/WMS/ReceiptManagement/AddInventoryAKC",
                            data: {
                                "ID": ID
                            },
                            async: "false",
...
$("#UpdateVolumeOK").live('click', function () {
    ...
    $.ajax({
        url: "/WMS/ReceiptManagement/UpdateReceiptVolume",
        type: "POST",
        dataType: "text",
        data: {
...
function printPick(ID) {
    layer.confirm('<font size="4">确认是否打印上架单？</font>', {
        ...
        if ($("#ProjectName").val() == "NIKE") {
            ...
            location.href = '/WMS/ReceiptManagement/PrintShelves_JT?rid=' + ID + "&Flag=1";
        }
        else {
            location.href = '/WMS/ReceiptManagement/PrintShelves_JT?rid=' + ID + "&Flag=1";
        }
...
function GenerateBarCode(receiptId) {
    //document.URL = '/WMS/ReceiptManagement/GenerateBarCode?ID=' + receiptId + "&ViewType=0";
