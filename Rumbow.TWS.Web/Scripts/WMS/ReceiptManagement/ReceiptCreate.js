$(document).ready(function () {
    ...
    $('#returnButton').live('click', function () {
        ...
        //$.post("/WMS/ReceiptManagement/Index", { "IndexViewModel": null, "Action": "查询" });
        //post('/WMS/ReceiptManagement/Index', { IndexViewModel: null, Action: '查询' });
    ...
    $('#NewPrint').live('click', function () {
        ...
        $.ajax({
            type: "Post",
            url: "/WMS/ReceiptManagement/GetSkuTotal",
            data: {
                "ID": id,
                "SKU": sku,
                "BoxNumber": '',
...
function AddReceiptAndReceiptDetail() {
    ...
    $.ajax({
        type: "Post",
        url: "/WMS/ReceiptManagement/AddReceiptAndReceiptDetail",
        data: {
            "JsonTable": JsonTable,
            "ASNNumber": AsnNumber,
            "ASNID": ASNID,
        ...
        success: function (data) {
            if (data.Errorcode == 1) {
                ...
                //showMsg("添加成功！", "10000");
                location.href = "/WMS/ReceiptManagement/ReceiptCreate/?ID=" + data.Result + "&ViewType=3&Flag=1&PageType=1"
            } else {
...
function EditReceiptAndReceiptDetail(ID) {
    ...
    $.ajax({
        type: "Post",
        url: "/WMS/ReceiptManagement/EditReceiptAndReceiptDetail",
        data: {
            "JsonTable": JsonTable,
            "ID": ID,
            "ReceiptDate": ReceiptDate,
...
function AddButtonAgain(CustomerID) {
    location.href = "/WMS/ReceiptManagement/ReceiptCreate/?CustomerID=" + CustomerID + "&ViewType=0"
}

function UpdateButton(ID) {
    location.href = "/WMS/ReceiptManagement/ReceiptCreate/?ID=" + ID + "&ViewType=2"
}
...
function SelectASN(CustomerID) {
    openPopup("", true, 1000, 600, '/WMS/ReceiptManagement/ASNQuery/?CustomerID=' + CustomerID, null, function (ASNID, CustomerID) {
        ...
        location.href = "/WMS/ReceiptManagement/ReceiptCreate/?ID=" + ASNID + "&ViewType=1&CustomerID=" + CustomerID
    });
}
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.ErrorCode == "1") {
                ...
                //    LODOP.PRINT();
                //}
                location.href = "/WMS/ReceiptManagement/ReceiptCreate/?ID=" + $("#ReceiptID").val() + "&ViewType=" + ViewType
            }
            else { }
