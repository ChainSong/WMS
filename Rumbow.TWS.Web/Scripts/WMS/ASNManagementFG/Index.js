$(document).ready(function () {
    ...
    $('select[id=ASNCondition_CustomerID]').live('change', function () {
        window.location.href = "/WMS/ASNManagementFG/Index/?customerID=" + $(this).val();
    });
    ...
    $('#portButton').live('click', function () {
        ...
        if (a == "") {
            ...
            window.location.href = "/WMS/ASNManagementFG/ExportOrder?IDs=" + a + "&CustomerID=" + $("#ASNCondition_CustomerID").val();
            return false;
        }
    });
    //生成上架库位
        ...
        if (a == "") {
            ...
            layer.confirm('<font size="4">是否生成上架库位？</font>', {
                ...
                if (IDs != "" && IDs != 0) {
                    $.ajax({
                        type: "POST",
                        url: "/WMS/ASNManagementFG/CreateShelfLocation",
                        data: {
                            "ID": IDs
                        },
                        async: "false",
    ...
    $("#printShelfLocation").live("click", function () {
        layer.confirm('<font size="4">确认是否打印箱标贴？</font>', {
            ...
            if (id == "") {
                ...
                window.location.href = '/WMS/ASNManagementFG/PrintBoxLabel?ids=' + id + "&type=3";
    ...
    //        title: ['提示', 'font-size:18px;']
    //    },
    //        location.href = '/WMS/ASNManagementFG/PrintASNNewBoxLabel?ids=' + str.substring(0, str.length - 1) + "&type=2"
    //    );
    //    return false;
    //}
    ...
    $('#ReceiptStatusStatis').live('click', function () {
        var href = "/WMS/ASNManagementFG/ReceiptStatusStatis?CustomerID=" + $("#ASNCondition_CustomerID").val() + "";
        layer.open({
            type: 2,
            title: '入库单状态统计',
            shadeClose: true,
...
var fileImportClick = function () {
    ...
    $.ajaxFileUpload({
        url: '/WMS/ASNManagementFG/ImportASN',
        secureuri: false,
...
//编辑操作
function edit(ID) {
    location.href = "/WMS/ASNManagementFG/ASNCreateOrEdit/?ID=" + ID + "&ViewType=2"
}
//单条转入库单操作
function Receipt(ID, CustomerID) {
    location.href = "/WMS/ReceiptManagementFG/ReceiptCreate/?ID=" + ID + "&ViewType=1&PageType=1&CustomerID=" + CustomerID
}
//单条取消操作
var IDs = '';
function ASNDelete(ID) {
    layer.confirm('<font size="4">是否取消？</font>', {
        ...
        if (IDs != "" && IDs != 0) {
            $.ajax({
                type: "POST",
                url: "/WMS/ASNManagementFG/ASNDelete",
                data: {
                    "ID": IDs
                },
                async: "false",
                success: function (data) {
                    showMsg("取消成功", "4000");
                    location.href = "/WMS/ASNManagementFG/Index/"
                },
                error: function (msg) {
                    showMsg("操作失败", "4000");
                }
...
function Complet(ID) {
    layer.confirm('<font size="4">是否完成？</font>', {
        ...
        if (ID != "" && ID != 0) {
            $.ajax({
                type: "POST",
                url: "/WMS/ASNManagementFG/Complet",
                data: {
                    "ID": ID
                },
                async: "false",
    ...
    layer.confirm('<font size="4">是否批量转入库单？</font>', {
        ...
        if ($("#resultTable tbody input[type='checkbox']:checked").size() == 0) {
            ...
            if (count == 0) {
                ...
                $.ajax({
                    type: "POST",
                    url: "/WMS/ASNManagementFG/InsertIntoReceiptAndReceiptDetails",
                    data: {
                        "ASNIDs": asnid
                    },
                    async: "false",
    ...
    layer.confirm('<font size="4">是否批量取消？</font>', {
        ...
        if ($("#resultTable tbody input[type='checkbox']:checked").size() == 0) {
            ...
            if (count == 0) {
                ...
                $.ajax({
                    type: "POST",
                    url: "/WMS/ASNManagementFG/ASNStatusReturn",
                    data: {
                        "asnnumberlist": asnid
                    },
                    async: "false",
    ...
    layer.confirm('<font size="4">是否批量完成？</font>', {
        ...
        if ($("#resultTable tbody input[type='checkbox']:checked").size() == 0) {
            ...
            if (count == 0) {
                ...
                $.ajax({
                    type: "POST",
                    url: "/WMS/ASNManagementFG/CompletALLSelect",
                    data: {
                        "asnid": asnid
                    },
                    async: "false",
...
function createasn() {
    ...
    location.href = "/WMS/ASNManagementFG/ASNCreateOrEdit/?ID=0" + "&customerID=" + customerid + "" + "&ViewType=1"
}

function ViewReceipt(ID, CustomerID) {
    $.ajax({
        type: "POST",
        url: "/WMS/ASNManagementFG/CountReceipt",
        data: {
            "ID": ID
        },
        async: "false",
        success: function (data) {
            if (data.length.toString() == "1") {
                location.href = "/WMS/ReceiptManagementFG/ReceiptCreate/?ID=" + data[0].ID + "&ViewType=3&CustomerID=" + CustomerID
            }
...
function addNew(td11, td22, td33) {
    ...
    td2.append($("<a href=" + '/WMS/ReceiptManagementFG/ReceiptCreate/?ID=' + td11 + '&ViewType=3>' + td22 + "</a>"));
    td3.append($("<b>" + td33 + "</b>"));
    row.append(td2);
    row.append(td3);
    table1.append(row);
