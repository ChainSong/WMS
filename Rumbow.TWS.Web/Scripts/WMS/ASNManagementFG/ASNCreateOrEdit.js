$(document).ready(function () {
    ...
    $('#returnButton').live('click', function () {
        ...
        if ($("#ReturnViewType").val() == "get") {
            ...
            //history.back();
            post('/WMS/ASNManagementFG/Index', { IndexViewModel: null, Action: '查询' });
        }
        //post('/WMS/ASNManagementFG/Index', { IndexViewModel: null, Action: '查询' });
    })
    ...
    $(".skuChildren").live('dblclick', function () {
        var obj = this;
        openPopup("AsnPop", true, 1000, 600, '/WMS/ProductFG/index/?flag=1&customerID=' + $("#CustomerID").val(), null, function (Sku, GoodsName, UPC) {
...
$('select[id=CustomerID]').live('change', function () {
    window.location.href = "/WMS/ASNManagementFG/ASNCreateOrEdit/?ID=0" + "&customerID=" + $(this).val() + "&ViewType=1";
});
...
//从view页面到edit页面
function edits(ID) {
    location.href = "/WMS/ASNManagementFG/ASNCreateOrEdit/?ID=0" + ID + "&ViewType=2"
}

function Receipt(ID, CustomerID) {
    location.href = "/WMS/ReceiptManagementFG/ReceiptCreate/?ID=" + ID + "&ViewType=1&CustomerID=" + CustomerID
}
...
function AddASNAndASNDetails() {
    ...
    $.ajax({
        type: "POST",
        url: "/WMS/ASNManagementFG/AddASNAndASNDetails",
        data: {
            "JsonTable": JsonTable,
        ...
        success: function (data) {
            if (data == "外部单号已存在") {
                ...
                location.href = "/WMS/ASNManagementFG/ASNCreateOrEdit/?ID=" + d + "&ViewType=3";
            } else {
...
function UpdateASNAndASNDetail(asndetailcount) {
    ...
    $.ajax({
        type: "Post",
        url: "/WMS/ASNManagementFG/UpdateASNAndASNDetails",
        data: {
            "JsonTable": JsonTable,
            "ID": ID,
            "ExternReceiptNumber": ExternReceiptNumber,
        ...
        async: "false",
        success: function (data) {
            location.href = "/WMS/ASNManagementFG/ASNCreateOrEdit/?ID=" + ID + "&ViewType=3";
            showMsg("修改成功!", "4000");
        },
        error: function (msg) {
            showMsg("修改失败！", "4000");
        ...
        for (var j = 1; j < row.length; j++) {
            ...
            for (var i = 0; i < col.length; i++) {
                ...
                if (tds[i].className.trim() != "TableColumnTitle" && tds[i].innerHTML.trim() != "") {
                    ...
                    } else if (tds[i].childNodes.length > 1 && tds[i].childNodes[1].type == 'text/javascript' && tds[i].childNodes.length > 1 && tds[i].childNodes[3].type == 'text') {
                        r += "\"" + tds[i].childNodes[3].id.trim() + "\"\:\"" + tds[i].childNodes[3].value + "\",";
                    }
                    else {
