...
function OpenGroupBatch(obj, RowID) {
    var isshow = "";
    if ($(obj).attr('class') == "row-details-close") {
    ...
    if ($(obj).parent().parent().next().attr('data-id') == RowID)//第一个子集行
    {
        if (isshow == "open") {
        ...
        ShowGroupBatch($(obj).parent().parent().next(), RowID, isshow);
    }
}
function ShowGroupBatch(obj, RowID,isshow) {
    if ($(obj).next().attr('data-id') == RowID) {
        if ($(obj).next().attr('data-type') == "batch")
        {
            if (isshow == "open") {
                $(obj).next().show();
            } else { $(obj).next().hide(); }
        }
        ShowGroupBatch($(obj).next(), RowID, isshow);
    }
}

function OpenGroupBox(obj, RowID) {
    var isshow = "";
    if ($(obj).attr('class') == "row-details-close") {
        $(obj).attr('class', 'row-details-open');
        isshow = "open";
    ...
    if ($(obj).parent().parent().next().attr('data-id') == RowID)//第一个子集行
    {
        ...
        ShowGroupBox($(obj).parent().parent().next(), RowID, isshow);
    }
}
function ShowGroupBox(obj, RowID, isshow) {
    if ($(obj).next().attr('data-id') == RowID) {
        if ($(obj).next().attr('data-type') == "box") {
            if (isshow == "open") {
                $(obj).next().show();
            } else { $(obj).next().hide(); }
            ShowGroupBox($(obj).next(), RowID, isshow);
        }        
    }
}

//function OpenGroupBox(obj) {
//    if ($(obj).attr('class') == "row-details-close") {
//        $(obj).attr('class', 'row-details-open');
//        $(obj).parent().parent().next().show();
//    } else {
