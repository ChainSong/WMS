$(document).ready(function () {
    ...
    $('select[id=CustomerID]').live('change', function () {
        window.location.href = "/WMS/InventoryManagement/AdjustAddorEditorView/?ID=0&ViewType=1&customerID=" + $(this).val();
    });
...
function addNew() {
    ...
    var td14 = $("<td class='ToGoodsType' style='width:80px;display:none;'></td>");
...
function AddAdjustAndAdjustDetail(IsSubmit) {
    ...
    if ($('#AdjustmentReason').val() != undefined) {
        AdjustmentReason = $('#AdjustmentReason').val();
    }
    let AdjustmentRemark = $("#AdjustmentRemark").val();
    //var AdjustmentRemark = $("#AdjustmentRemark").val();
    /////
    if (AdjustmentRemark == "") {
        showMsg("调整备注不可为空!", 4000);
        return;
    }
                        ...
                        success: function (data) {
                            ...
                            if (data.indexOf("提交成功") >= 0) {
                                ...
                                if (AdjustmentType == "库存调整单") {
                                    window.location.href = "/WMS/InventoryManagement/AdjustAddorEditorView/?ID=" + d + "&ViewType=0&Adjustflag=调整打印标记";
                                }
                                else {
                                    window.location.href = "/WMS/InventoryManagement/AdjustAddorEditorView/?ID=" + d + "&ViewType=0";
                                }
...
function addangian() {
    var customerID = $('#CustomerID').val();
    location.href = "/WMS/InventoryManagement/AdjustAddorEditorView/?ID=0" + "&customerID=" + customerID + "&ViewType=1"
}
//新增页面提交
