$(document).ready(function () {
    ...
    $('select[id=Condition_CustomerID]').live('change', function () {
        ...
        window.location.href = "/WMS/ShelvesManagement/Index/?customerID=" + $(this).val() + "&hideActionButton=" + hiddenActionButton + "&showEditRelated=" + showEditRelated;
    });
    ...
    $('#ExeclShelvesSingle').live('click', function () {
        ...
        if (a == "") {
            ...
            window.location.href = "/WMS/ShelvesManagement/ExportReceiptReceving?IDs=" + a + "&CustomerID=" + $("#SearchCondition_CustomerID").val() + "&type=1";
            return false;
        }
    });
    function demo() {
        ...
        form.attr('action', '/WMS/ShelvesManagement/ExportReceiptReceiving');
        var input1 = $('<input>');
...
var fileImportClick = function () {
    ...
    $.ajaxFileUpload({
        url: "/WMS/ShelvesManagement/ImputEcecl",
        secureuri: false,
...
$("#statusBack").live('click', function () {
    ...
    $.ajax({
        url: "/WMS/ShelvesManagement/BackStatus",
        type: "POST",
        data: {
            "ID": sql,
        ...
        success: function (data) {
            ...
            if (data == '') {
                showMsg("回退成功！", 1000)
                setTimeout("window.location.href = '/WMS/ShelvesManagement/Index'", 1000);
            } else {
...
function CheckReceiving(id, chktype) {
    ...
    $.ajax({
        type: "POST",
        url: "/WMS/ShelvesManagement/CheckReceiptReceiving",
        data: {
            "id": id,
        },
        dataType: "json",
...
function CheckNoReceiving(id, chktype) {
    ...
    $.ajax({
        type: "POST",
        url: "/WMS/ShelvesManagement/CheckRFReceiptReceiving",
        data: {
            "id": id,
        },
        dataType: "json",
...
function ExportDiffrent(id, chktype) {
    var resualt = false;
    window.location.href = "/WMS/ShelvesManagement/ExportDiffrent?id=" + id;
}
//显示结果
...
function AddInventory(id, receiptnumber, oneself) {
    layer.confirm('<font size="4">是否加入库存？</font>', {
        ...
        if (CheckReceiving(id, 1)) {
            $.ajax({
                type: "POST",
                url: "/WMS/ShelvesManagement/AddInventory",
                data: {
                    "id": id,
                },
                async: "false",
            ...
            if ($("#ProjectName").val() == "NIKE") {
                layer.confirm('<font size="4">存在差异，是否加入库存并生成冻结单？</font>', {
                    ...
                    $.ajax({
                        type: "POST",
                        url: "/WMS/ShelvesManagement/AddInventoryWithFreeze", //Nike差异入库自动生成冻结单
                        data: {
                            "id": id,
                        },
                        async: "false",
...
function ShelvesLink(ShelvesLink, Status) {
    window.location.href = "/WMS/ShelvesManagement/ReceiptReceivingDetail/?RID=" + ShelvesLink + "&Status=" + Status;
}
