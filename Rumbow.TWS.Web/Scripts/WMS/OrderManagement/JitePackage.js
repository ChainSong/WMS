$(document).ready(function () {
    ...
    $("#submitButton").live('click', function () {
        ...
        $(".counts").each(function (a, b) {
            if ($(b).children()[0].rows[2].cells[1].children.Length.value == "" || $(b).children()[0].rows[2].cells[3].children.Width.value == "" ||
                $(b).children()[0].rows[2].cells[5].children.Height.value == "" || $(b).children()[1].rows[1].cells[0].children.SKU.value == "" ||
                $(b).children()[1].rows[1].cells[2].children.GoodsType.value == "" || $(b).children()[1].rows[1].cells[4].children.Qty.value == "") {
                flag = 1;
                showMsg("有空值！", 4000);
                return;
            }
            ...
            //    $(b).children()[5].rows[1].cells[2].children.GoodsType.value == "" || $(b).children()[5].rows[1].cells[4].children.Qty.value == "") {
            //    flag = 1;
            //    showMsg("有空值！", 4000);
            //    return;
            //}
            ...
            var table1 = tables[0]; //tables[4];
            var table2 = tables[1]; //tables[5];
            var txt = "";
            var r = "{";
        ...
        var qtylast = parseInt($("#lastcount")[0].innerHTML.toString().substring(5, $("#lastcount")[0].innerHTML.length));
        if (qtylast != 0) {
            CheckDiffent();
            return;
        }
        ...
        if (flag == 0) {
            $.ajax({
                type: "Post",
                url: "/WMS/OrderManagement/NikePackage",
                data: {
                    "ID": $("#OrderID").val(),
    ...
    $("#packageButton").live('click', function () {
        layer.confirm('<font size="4">提交后不能继续包装，确认是否提交？</font>', {
            ...
            if (flag == 0) {
                $.ajax({
                    ...
                    success: function (data) {
                        if (data == "") {
                            showMsg("包装交接完成！", 4000);
                            location.href = "/WMS/OrderManagement/NikePackage/?ID=" + $("#OrderID").val();
                        }
                        else {
    ...
    $("#addButton").live('click', function () {
        ...
        if (BoxNum.trim().length = 0 || BoxNum.trim() == "") {
            return;
        }
        var OrderId = $("#OrderID").val();
        var obj = $('#packagesModel');
        var newObj = $('#packagesModel').clone(true, true);
        $(newObj).attr('style', 'margin-bottom:40px;border:2px solid #e8eef4');
        $(newObj).attr('class', 'counts');
        $(newObj).find('#PackageNumber').val(BoxNum);
        $(newObj).find('.boxSerial').html("第" + sums + "箱");
        ...
        var htm = "";
        htm += "<div id='operateTD" + BoxNum + "' style=\"display:none;position:absolute;left:-300px\" class=\"ddiv\">";
        if (customerName == "吉特WMS") {
            htm += "<div class=\"CheckOutboundOrder\" data-id=\"\" style=\"text-align:center; width:100px; float:left; border:solid 1px ;background-color:#f0ad4e;border-radius:3px;cursor:pointer;color:white;\" onclick=\"printboxmDiv('" + BoxNum + "','B','" + OrderId + "','DP')\">打印德邦面单</div>";
        }
        else {
            htm += "<div class=\"CheckOutboundOrder\" data-id=\"\" style=\"text-align:center; width:100px; float:left; border:solid 1px ;background-color:#f0ad4e;border-radius:3px;cursor:pointer;color:white;\" onclick=\"printboxmDiv('" + BoxNum + "','B','" + OrderId + "','YTO')\">打印圆通面单</div>";
            htm += "<div class=\"CheckOutboundOrder\" data-id=\"\" style=\"text-align:center; width:100px; float:left; border:solid 1px ;background-color:#f0ad4e;border-radius:3px;cursor:pointer;color:white;\" onclick=\"printboxmDiv('" + BoxNum + "','B','" + OrderId + "','YD')\">打印韵达面单</div>";
        }
        htm += '</div>';
        htm += "<div style='text-align:center; width:75px; border:solid 1px ;border-radius:3px;cursor:pointer;color:white;background-color:#5bc0de' onmouseover=\"ShowsIn('" + BoxNum + "',this)\">打印</div>";

        $('.counts').first()[0].children[0].children[0].children[0].children[1].innerHTML = htm;
        $('.ddiv').on('mouseenter', function () {
            $(this)[0].style.display = "";
        })
        $('.ddiv').on('mouseleave', function () {
        ...
        $($('.counts').first()[0].children[0]).find('#ScanSKU').focus();
        boxchanged($('select[id=box]')[1]);
        //var sums = 0;
        //if ($(".counts").length > 0) {
        //    var boxtext = $(".counts")[$(".counts").length - 1].innerText.trim();
        ...
        //var h = $(document).height() - $(window).height();
        //$(document).scrollTop(h);
        //$(".counts").first()[0].children[4].childNodes[1].childNodes[0].innerHTML += "<td colspan='2'><div id='operateTD" + BoxNum + "' style='float:right;display:none;width:250px;' class='ddiv' >"
        //    + "<div class='CheckOutboundOrder' data-id='' style='text-align:center; width:150px; float:left; border:solid 1px ;background-color:#f0ad4e;border-radius:3px;cursor:pointer;color:white; '  onclick=\"printboxmDiv('" + BoxNum + "',B,'" + OrderId + "')\">打印快递面单</div> </div> </td><td><div style='text-align:center; width:75px; border:solid 1px ;border-radius:3px;cursor:pointer;color:white;background-color:#5bc0de'  onmouseover =\"ShowsIn('" + BoxNum + "',this)\" >打印</div></td>"
    ...
    $("#ScanSKU").live('keydown', function () {
        if (event.keyCode == 13) {
            ...
            //根据规则过滤唯一码
            if ($("#CustomerName").val().toString().indexOf("马天奴WMS") > -1) {
                var sku = $(this).val();
                sku = FilterUniqueCode(sku);
                $(this).val(sku);
            }
            else { }
            ...
            inputsku.select();
            var thistr = inputsku.parent().parent().parent().parent().parent()[0].children.resultTableModel;
            if (thistr == undefined) {
                thistr = $(inputsku.parent().parent().parent().parent().parent()[0].children.resultTable)[0];
            }
            ...
            if (rows_count == 1 && $(thistr.rows[1]).children().eq(0).find("input").val().trim() == "") {
                ...
                //})
                QtyChanged();
                BoxQtySum($(this).parent().parent().parent().parent().parent());//把父节点传进方法计算此箱总数
                return true;
            }
            var Isexist = false;
            var Isexistline = 0;
            ...
            if (Isexist) {
                ...
                if (CheckSKU(inputsku.val())) {
                    ...
                    thistr = $(inputsku.parent().parent().parent().parent().parent()[0].children.resultTable)[0];
                    if (thistr == undefined) {
                        thistr = inputsku.parent().parent().parent().parent().parent()[0].children.resultTableModel;
                    }
                    ...
                    QtyChanged();
                    BoxQtySum($(this).parent().parent().parent().parent().parent());
                    return true;
                }
                else {
                    //showMsg("SKU不存在!", "4000");
...
function BoxQtySum(par) {
    ...
    var boxQty = 0;
    var text = "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 该箱总件数：";//文本前摇
    for (var i = 0; i < $(par).children()[1].rows.length; i++) {
        if (i != 0) {//表头不计
            if ($(par).children()[1].rows[i].cells[4].children.Qty.value != "") {
                if (parseInt($(par).children()[1].rows[i].cells[4].children.Qty.value) != NaN) {
                    boxQty += parseInt($(par).children()[1].rows[i].cells[4].children.Qty.value);
                }
            }
    ...
    $(par).children()[0].children[0].children[0].children[0].children[3].innerHTML = text + boxQty;
}
...
function QtyChanged(obj) {
    ...
    qty = qty - qty;
    $(".counts").each(function (a, b) {
        for (var i = 1; i < $(b).children()[1].rows.length; i++) {
            var $tr = $(b).children()[1].rows[i];
            var $td = $($tr).find('td');
            if (parseInt($td.eq(4).find('input').val()) != NaN && $td.eq(4).find('input').val() != '') {
                qty += parseInt($td.eq(4).find('input').val());
            }
        }
    ...
    $(".counts").each(function (a, b) {
        for (var i = 1; i < $(b).children()[1].rows.length; i++) {
            var $tr = $(b).children()[1].rows[i];
            var $td = $($tr).find('td');
            if ($td.eq(0).find('input').val() == singleSKU) {
                if (parseInt($td.eq(4).find('input').val()) != obj) {
                    singleQty += parseInt($td.eq(4).find('input').val());
                }
            }
        }
        //for (var i = 0; i < $(b).children()[5].rows.length; i++) {
...
function CheckQTYOver(obj) {
    ...
    $(".counts").each(function (a, b) {
        for (var i = 0; i < $(b).children()[1].rows.length; i++) {
            if (i != 0) {
                if ($(b).children()[1].rows[i].cells[0].children.SKU.value == obj) {
                    var isexist = false;
                    if (parseInt($(b).children()[1].rows[i].cells[4].children.Qty.value) != obj)
                        totalqty += parseInt($(b).children()[1].rows[i].cells[4].children.Qty.value);
                }
            }
        }
        //for (var i = 0; i < $(b).children()[5].rows.length; i++) {
...
function CheckDiffent() {
    ...
    $(".counts").each(function (a, b) {
        for (var i = 0; i < $(b).children()[1].rows.length; i++) {
            if (i != 0) {
                if ($(b).children()[1].rows[i].cells[0].children.SKU.value != "") {
                    var isexist = false;
                    if (parseInt($(b).children()[1].rows[i].cells[4].children.Qty.value) != NaN) {
                        $.each(packagelist, function (index, item) {
                            if (item[0] == $(b).children()[1].rows[i].cells[0].children.SKU.value) {
                                packagelist[index][1] = parseInt(packagelist[index][1]) + parseInt($(b).children()[1].rows[i].cells[4].children.Qty.value);
                                isexist = true;
                                return false;
                            }
                        });
                        if (!isexist) {
                            ...
                            packagelist[indexcount][0] = $(b).children()[1].rows[i].cells[0].children.SKU.value;
                            packagelist[indexcount][1] = parseInt($(b).children()[1].rows[i].cells[4].children.Qty.value);
                        }
                    }
                }
            }
        ...
        $.each(packagelist, function (index2, item2) {
            if (item.SKU == item2[0]) {
                ...
                if (item.Qty == item2[1]) {
                    ...
                    chayilist[chayileng][4] = item.str6;
                    chayilist[chayileng][5] = item.str7;
                }
                return false;
            }
        });
...
function showmsg(data) {
    if ($("#showdata2")[0].style.display == 'none') {
        ...
        $("#showtable2").append("<tr><td style='font-weight:700;'>SKU</td><td style='font-weight:700;'>订单数量</td><td style='font-weight:700;'>包装数量</td><td style='font-weight:700;color:red;'>差异数量</td></tr>");
        $.each(data, function (index, item) {
            var chayiqty = item[3];
            if (chayiqty > 0) {
                $("#showtable2").append("<tr><td >" + item[0] + "</td><td>" + item[1] + "</td><td>" + item[2] + "</td><td style='color:red;'>" + "+" + (item[3]) + "</td></tr>")
            }
            else {
                $("#showtable2").append("<tr><td >" + item[0] + "</td><td>" + item[1] + "</td><td>" + item[2] + "</td><td style='color:red;'>" + (item[3]) + "</td></tr>")
            }
...
function addNew(o) {
    ...
    td4.append($("<input type='text'  class='form-control' style='width:100%' id='Qty' readonly='true' onchange='QtyChanged(this)' /> "));
...
function deleteDiv(obj) {
    layer.confirm('<font size="4">确认是否删除本箱？</font>', {
        ...
        //}
        //packnum = $(obj.parentNode.parentNode.parentNode)[0].getElementsByTagName("td")[12].children[0].value;
        packnum = $(obj).parent().parent().parent().find('#PackageNumber').val();
        console.log(packnum)
        var isOk = DeletePackInfo(packnum);
        if (isOk) {
            //var s = $(obj.parentNode.parentNode.parentNode.parentNode.parentNode)[0].remove();
            $(obj).parent().parent().parent().parent().parent().remove();
            var bcount = $(".counts").length;
            $('.counts').each(function (a, b) {
                $($(b).children()[0]).find('.boxSerial').html("第" + (bcount - a) + "箱");
            })
            //$(".counts").each(function (a, b) {
...
function printDiv(id, type, OrderID) {
    ...
    if ($("#CustomerName").val().toString().indexOf("NIKE") > -1) {
        ...
        if (($("#CustomerID").val().toString().indexOf("103") > -1)) {
            var href = "/WMS/NikeOSRBJPrint/PrintPod_TH?id=" + id + "&type=" + type + "&OrderID=" + OrderID;
        }
        else {
...
function ShowsIn(ID, obj) {
    if ($("#operateTD" + ID)[0].style.display != "" && $("#operateTD" + ID)[0].style.display != "block") {
        ...
        var elem = $(obj).prev().children.length;
        $("#operateTD" + ID).animate({
            width: elem * 100 + 75 + 'px',
            height: "30px",
            paddingRight: "show",
            paddingLeft: "show",
            marginRight: "show",
        ...
        $(obj).prev()[0].style.left = (-100) * elem + 'px';
    }
    //$("#operateTD" + ID)[0].style.display = "";
}
//打印箱清单
function printboxDiv(id, type, OrderID) {
    if ($("#CustomerName").val().toString().indexOf("NIKE-OSR") > -1) {
        var href = "/WMS/NikeOSRBJPrint/PrintBox?id=" + id + "&type=" + type + "&OrderID=" + OrderID;
    }
    else if (($("#CustomerName").val().toString().indexOf("NIKE-NFS") > -1) || ($("#CustomerName").val().toString().indexOf("NIKE-CD") > -1) || ($("#CustomerName").val().toString().indexOf("NIKE-SH") > -1)) {
        var href = "/WMS/NikeNFSPrint/PrintBox?id=" + id + "&type=" + type + "&OrderID=" + OrderID;
    }
    else if (($("#CustomerID").val().toString().indexOf("103") > -1)) {
        var href = "/WMS/NikeOSRBJPrint/PrintBox_TH?id=" + id + "&type=" + type + "&OrderID=" + OrderID;
    }
    else {
        var href = "/WMS/NikeOSRBJPrint/PrintBox?id=" + id + "&type=" + type + "&OrderID=" + OrderID;
    }
...
//打印快递面单
function printboxmDiv(id, type, OrderID, express) {
    var packageNumBegin = Number(document.getElementById("startpackage").value);
    var packageNumEnd = Number(document.getElementById("endpackage").value);
    var maxPackage = $('#maxPackage').val();
    var maxPackageNum = Number(maxPackage);

    if (packageNumBegin <= packageNumEnd && packageNumBegin <= maxPackageNum && packageNumEnd <= maxPackageNum && !(packageNumBegin == '') && !(packageNumEnd == '') && packageNumBegin > 0 && packageNumEnd > 0) {
        ///快递区分
        //批量打印，检查所有箱快递是否同一家
        var href = '';
        if (express == "ALL") {
            var expressArr = [];
            var exp = $('#label_ExpressCompany').text();//若规定快递
            if (exp != '')
                expressArr.push(exp);
            
            $('.counts').each(function (i, item) {
                var div = $(item);
                var table = div[0].children[0];
                var tr = $(table).find('tr')[1];
                var td = $(tr).find('td');
                var ec = td[3].innerText;
                if ($.inArray(ec, expressArr) < 0 && ec != '') {
                    expressArr.push(ec);
                }
            });
            if (expressArr.length > 1) {
                layer.alert('包装信息存在多个快递公司，无法批量打印', {
                    icon: 2
                })
                return false;
            }
                ...
                if (expressArr.length == 1) {
                    express = getExpress(expressArr[0]);
                }
                else {
                    layer.alert('快递公司未知，请先至少打印一箱！', {
                        icon: 2
                    })
                    return false;
                }
        ...
        //快递面单
        if (express == 'DP')
            href = "/WMS/OrderManagement/PrintExpressDeppon?id=";
        else if (express == 'YTO')
            href = "/WMS/OrderManagement/PrintExpressYto?BoxNumber=";
        else if (express == 'YD')
            href = "/WMS/OrderManagement/PrintExpressYd?id=";

        $.ajax({
            type: "Post",
            url: "/WMS/NikeOSRBJPrint/PrintExpressBillJite",
            data: {
                "id": id,
                "type": type,
                "OrderID": OrderID,
                "packageNumBegin": packageNumBegin,
                "packageNumEnd": packageNumEnd,
                express: express
            },
            dataType: "json",
            async: false,
            success: function (data) {
                if (data.Code == "1") {
                    $('.counts').each(function (i, item) {
                        ...
                        var tr = $(table).find('tr')[1];
                        var td = $(tr).find('td');
                        var expressCompany = $(td)[3].innerText;
                        var expressNum = $(td)[4].innerText;
                        if (expressCompany == '' || expressNum == '') {
                            $(td)[3].innerText = data.Result[data.Result.length - i].ExpressCompany;
                            $(td)[4].innerText = data.Result[data.Result.length - i].ExpressNumber;
                        }
                    })

                    var s = "";
                    $.each(data.Result, function (i, item) {
                        s += "'" + item.PackageNumber + "',";
                    })
                    //var href = "/WMS/OrderManagement/PrintExpressDeppon?id=" + s.substring(0, s.length - 1);
                    href += s.substring(0, s.length - 1);
                    layer.open({
                        type: 2,
                        title: '打印快递面单',
                        shadeClose: true,
                ...
                } else if (data.Code == "0" || data.Code == "999") {
                    alert(data.Result);
                } else {
                    //
                }
        ...
        alert("输入数字，起始箱号应小于结束箱号，数据取值不要超过范围！");
    }
}
...
function ExportBoxDetails(id, type, OrderID) {
    if ($("#CustomerName").val().toString().indexOf("NIKE-OSR") > -1) {
        window.location.href = "/WMS/NikeOSRBJPrint/ExportBoxDetails?id=" + id + "&type=" + type + "&OrderID=" + OrderID;
    }
    else if ($("#CustomerName").val().toString().indexOf("NIKE-NFS") > -1) {
        window.location.href = "/WMS/NikeOSRBJPrint/ExportBoxDetails?id=" + id + "&type=" + type + "&OrderID=" + OrderID;
    }
    else if ($("#CustomerName").val().toString().indexOf("NIKE-CD") > -1 || $("#CustomerName").val().toString().indexOf("NIKE-WH") > -1 || $("#CustomerName").val().toString().indexOf("NIKE-XA") > -1) {
        window.location.href = "/WMS/NikeOSRBJPrint/ExportBoxDetails?id=" + id + "&type=" + type + "&OrderID=" + OrderID;
    }
    else if ($("#CustomerName").val().toString().indexOf("YXDRSHH") > -1) {
        ...
        window.location.href = "/WMS/NikeOSRBJPrint/ExportBoxDetails_TH?id=" + id + "&type=" + type + "&OrderID=" + OrderID;
    }
}
        ...
        success: function (data) {
            if (data != "") {
                ...
                layer.open({
                    ...
                    , content: '<div style="padding: 20px 80px;">新增箱失败，请重试！</div>'
                    , btn: '关闭'
                    , btnAlign: 'c' //按钮居中
                    , shade: 1 //0不显示遮罩 1显示遮罩
                    , yes: function () {
                        layer.closeAll();
                    }
                });
...
function ScanCheck() {
    ...
    $(".counts").each(function (a, b) {
        if ($(b).children()[0].rows[2].cells[1].children.Length.value == "" || $(b).children()[0].rows[2].cells[3].children.Width.value == "" ||
            ...
            $(b).children()[1].rows[1].cells[2].children.GoodsType.value == "" || //$(b).children()[5].rows[1].cells[3].children.GoodsName.value == "" ||
            $(b).children()[1].rows[1].cells[4].children.Qty.value == "") {
            flag = 1;
            showMsg("有空值！", 4000);
            return;
        }
    ...
    if (flag == 0) {
        $.ajax({
            ...
            success: function (data) {
                if (data == "") {
                    var id = JSON.parse(jsonString)[0].PackageNumber.toString();
                    var type = "0";
                    var OrderID = $("#OrderID").val().toString();
                    var packageNumBegin = Number(document.getElementById("startpackage").value).toString();
                    var packageNumEnd = Number(document.getElementById("endpackage").value).toString();
                    var href = "";
                    if ($("#label_OrderType").text().indexOf("CSC-门店间互相调拨") != -1 || $("#label_OrderType").text().indexOf("CSC-转货出库") != -1 || $("#label_OrderType").text().indexOf("CSC-退大仓") != -1) {
                        href = "/WMS/NikeOSRBJPrint/PrintBoxSHCORTW?id=" + id + "&type=" + type + "&OrderID=" + OrderID + "&packageNumBegin=" + packageNumBegin + "&packageNumEnd=" + packageNumEnd + "&Flag=1";
                    } else {
                        href = "/WMS/NikeOSRBJPrint/PrintBoxSH?id=" + id + "&type=" + type + "&OrderID=" + OrderID + "&packageNumBegin=" + packageNumBegin + "&packageNumEnd=" + packageNumEnd + "&Flag=1";
                    }
                    layer.open({
                        type: 2,
                        title: '打印箱唛',
...
//定位箱号的位置
function FilterUniqueCode(sku) {
    var tempsku = sku.substring(sku.length - 5, sku.length);
    var reg = /[a-zA-Z]{1}[0-9]{4}/;
    if (reg.test(tempsku)) {
        tempsku = sku.substring(0, sku.length-4);
    } else {
        tempsku = sku;
    }
    return tempsku;
