...
$(document).ready(function () {
    ...
    $('select[id=box]').live('change', function () {
        ...
        var NetWeights = array[0] * array[1] * array[2] / 1000000;
        $(this).parent().parent().next().next().find('#NetWeight').val(NetWeights.toString().substring(0, 7));
        //$(str).parent().parent().next().next().find($('#NetWeight').val(NetWeights.toString().substring(0, 7)));
    });
    ...
    $("#submitButton").live('click', function () {
        ...
        $(".counts").each(function (a, b) {
            if ($(b).children()[4].rows[1].cells[1].children.Length.value == "" || $(b).children()[4].rows[1].cells[3].children.Width.value == "" ||
                $(b).children()[4].rows[1].cells[5].children.Height.value == "" || $(b).children()[4].rows[2].cells[3].children.NetWeight.value == "" ||
                $(b).children()[4].rows[2].cells[5].children.GrossWeight.value == "" ||
                $(b).children()[5].rows[1].cells[0].children.SKU.value == "" || //$(b).children()[5].rows[1].cells[1].children.UPC.value == "" ||
        ...
        if (flag == 0) {
            $.ajax({
                type: "Post",
                url: "/WMS/OrderManagement/NikeTHPackage",
                data: {
                    "ID": $("#OrderID").val(),
            ...
            if (flag == 0) {
                $.ajax({
                    ...
                    success: function (data) {
                        if (data == "") {
                            showMsg("包装交接完成！", 4000);
                            location.href = "/WMS/OrderManagement/NikeTHPackage/?ID=" + $("#OrderID").val();
                        }
                        else {
...
function showmsg(data) {
    if ($("#showdata2")[0].style.display == 'none') {
        ...
        $("#showtable2").append("<tr><td>SKU</td><td>Article Size</td><td>BU</td><td>订单数量</td><td>包装数量</td><td>差异数量</td></tr>");
        $.each(data, function (index, item) {
            var chayiqty = item[3];
            if (chayiqty > 0) {
                $("#showtable2").append("<tr><td >" + item[0] + "</td><td >" + item[4] + "</td><td >" + item[5] + "</td><td>" + item[1] + "</td><td>" + item[2] + "</td><td >" + "+" + (item[3]) + "</td></tr>")
            }
            else {
                $("#showtable2").append("<tr><td >" + item[0] + "</td><td >" + item[4] + "</td><td >" + item[5] + "</td><td>" + item[1] + "</td><td>" + item[2] + "</td><td >" + (item[3]) + "</td></tr>")
...
function printboxDiv(id, type, OrderID) {
    ...
    //}
    else if ($("#CustomerName").val().toString().indexOf("NIKE-Return") > -1) {
...
function printboxmDiv(id, type, OrderID) {
    ...
    if (packageNumBegin <= packageNumEnd && packageNumBegin <= maxPackageNum && packageNumEnd <= maxPackageNum && !(packageNumBegin == '') && !(packageNumEnd == '') && packageNumBegin > 0 && packageNumEnd > 0) {
        if ($("#CustomerName").val().toString().indexOf("NIKE-WH") > -1) {
            var href = "/WMS/NikeOSRBJPrint/PrintBoxWH?id=" + id + "&type=" + type + "&OrderID=" + OrderID + "&packageNumBegin=" + packageNumBegin + "&packageNumEnd=" + packageNumEnd;
        }
        else if ($("#CustomerName").val().toString().indexOf("NIKE-CD") > -1) {
            var href = "/WMS/NikeOSRBJPrint/PrintBoxCD?id=" + id + "&type=" + type + "&OrderID=" + OrderID + "&packageNumBegin=" + packageNumBegin + "&packageNumEnd=" + packageNumEnd;
        }
        else if ($("#CustomerName").val().toString().indexOf("NIKE-XA") > -1) {
            var href = "/WMS/NikeOSRBJPrint/PrintBoxXA?id=" + id + "&type=" + type + "&OrderID=" + OrderID + "&packageNumBegin=" + packageNumBegin + "&packageNumEnd=" + packageNumEnd;
        }
        else if ($("#CustomerName").val().toString().indexOf("NIKE-SH") > -1) {
            //判断订单类型
            if ($("#label_OrderType").text().indexOf("CSC-门店间互相调拨") != -1 || $("#label_OrderType").text().indexOf("CSC-转货出库") != -1 || $("#label_OrderType").text().indexOf("CSC-退大仓") != -1) {
                var href = "/WMS/NikeOSRBJPrint/PrintBoxSHCORTW?id=" + id + "&type=" + type + "&OrderID=" + OrderID + "&packageNumBegin=" + packageNumBegin + "&packageNumEnd=" + packageNumEnd;
            } else {
                var href = "/WMS/NikeOSRBJPrint/PrintBoxSH?id=" + id + "&type=" + type + "&OrderID=" + OrderID + "&packageNumBegin=" + packageNumBegin + "&packageNumEnd=" + packageNumEnd;
            }
        }
        else if ($("#CustomerName").val().toString().indexOf("NIKE-Return") > -1) {
            var href = "/WMS/NikeOSRBJPrint/PrintBoxm_TH?id=" + id + "&type=" + type + "&OrderID=" + OrderID + "&packageNumBegin=" + packageNumBegin + "&packageNumEnd=" + packageNumEnd;
        }
        else {
...
//    window.location.href = "/WMS/YXDRPackagePrint/PrintCustomsCarton?id=" + id + "&type=" + type + "&OrderID=" + OrderID;
//}
