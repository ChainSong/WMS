...
function Save() {
    ...
    $.ajax({
        ...
        error: function (msg) {
            document.getElementById('AssociatedTable').innerHTML = '<h1 style="color:green;">'+msg.val+'</h1>';
        }
    });
}
function checkSKU() {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (obj[0].AssociatedStatus == "0") {
                    if (obj[0].OrderStatus != null) {
                        document.getElementById('AssociatedTable').innerHTML = '<h1 style="color:green;">当前状态 ' + obj[0].OrderStatus + ' 不允许包装，请重新扫描！</h1>';
                        $("#PackageCollection_Scan").select();
                    } else {
                    ...
                    if (obj[0].PackeStatus == "0") {
                        ...
                        document.getElementById('AssociatedTable').innerHTML = '<h1 style="color:red;">已包装，请勿重复操作！</h1>';
                        $("#PackageCollection_Scan").select();
                    }
                }
            },
