...
namespace Runbow.TWS.RFScan.Common
{
    public class ApplicationConfigHelper
    {
        ...
        public static void RefreshGetWarehouseLocationList()
        {
            long a = 0;
            WarehouseService service = new WarehouseService();
            var AreaList = service.GetWarehouseLocationList(a).Result;
            lock (lockobject)
            {
                CacheHelper.Remove("WarehouseLocationList");
                CacheHelper.Insert("WarehouseLocationList", AreaList);
            }
        }
        ...
        /// <param name="ID"></param>
        /// <returns></returns>
        public static IEnumerable<ProductStorer> GetProductStorerList()
        {
            string SqlWhere = "";
            var list = (IEnumerable<ProductStorer>)CacheHelper.Get("ProductStorerList", () =>
            {
        ...
        /// 刷新产品表
        /// </summary>
        public static void RefreshGetProductStorerList()
        {
            string SqlWhere = "";
            ProductService service = new ProductService();
            var AreaList = service.GetProductStorerList(SqlWhere).Result;
            lock (lockobject)
            {
                CacheHelper.Remove("ProductStorerList");
                CacheHelper.Insert("ProductStorerList", AreaList);
            }
        }
        //将库位库区缓存起来
        ...
        public static IEnumerable<ProductStorer> GetALLProductStorerList(long? CustomerID)
        {
            var list = (IEnumerable<ProductStorer>)CacheHelper.Get("ProductStorerList", () =>
            {
                ProductService service = new ProductService();
                var AreaList = service.GetProductStorerList(" and p.storerid=" + CustomerID.ToString()).Result;
        ...
        public static IEnumerable<ProductStorer> GetALLProductStorerList(long? CustomerID,string UPC)
        {
            var list = (IEnumerable<ProductStorer>)CacheHelper.Get("ProductStorerList", () =>
            {
                ProductService service = new ProductService();
                var AreaList = service.GetProductStorerList(" and p.storerid=" + CustomerID.ToString() + " and pd.UPC='" + UPC+"'").Result;
        ...
        /// 获取WMS中存储过程 配置缓存
        /// </summary>
        public static IEnumerable<WMS_Config_Type> GetWMS_ConfigType(string types,int ProjectID,int CustomerID,int WarehouseID)
        {
